...
| Flag | Type | Default | Description |
|---|---|---|---|
| `--timeout <DUR>` | seconds-int or duration string | `300` (5 min) | Per-file timeout. `0` disables. Accepts `"500ms"`, `"30s"`, `"10m"`, `"2h"`. Time queued in the pool is not counted. |
...
Run dynamic profiles' [`build`](~provium/configuration/provium-toml#build) commands without booting anything. With a `PROFILE` argument, builds just that profile (and errors with `provium prepare: profile \`<name>\` not found in provium.toml` if it isn't declared); with no argument, builds every profile that declares a `build` command, in name order, stopping at the first failure.
