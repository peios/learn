...
Boots the guest. Returns `self` so calls chain (`provium:vm("a", "peios"):boot()`). The optional `opts` table (`kernel_cmdline`, `kernel_cmdline_append`, `rng_seed`, `initial_time`, `files`, `agent_timeout`) merges per field into the VM's pending boot options. Errors if already booted.
