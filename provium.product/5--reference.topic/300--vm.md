...
[`vm:attach_disk`](#vmattach-diskopts), whose attachment is created
...
The same `bufs` / `ptrs` / `nested` options are available on [`worker:syscall`](~provium/reference/worker#workersyscallnr-).
