...
title: VM
type: reference
description: The VM userdata — lifecycle, run and run_async, file I/O, syscalls and ioctls, disks, NICs, console, clock, workers, batch, snapshots, and RunResult.
related:
...
| `lab:vm("name", "profile", opts)` | Same, with sizing opts (`memory`, `cpus`). See [boot opts](#boot-opts). |
...
| `kernel_cmdline_append` | `vm:boot` opts | string | Tokens appended after the resolved command line (the profile's, or `kernel_cmdline`). Use it to add one parameter — `maxcpus=1`, a module option — without restating the profile's line; where a token repeats one already present, the kernel takes the last, so the appended one wins. |
...
| `files` | `vm:boot` opts | array of `{path=string, content=string, mode=int}` | Files to place in the guest's filesystem before init runs. `mode` is the permission mask and defaults to `0644`; pass `0x1ed` (`0755`) for something the guest will execute. See [Injected files](#injected-files). |
| `disks` | `vm:boot` opts | array of `{path=string, id=string, readonly=bool}` | Block devices attached for this boot only, after the profile's own. `id` defaults to `boot<N>` by position; `readonly` defaults to `false`. See [Boot disks](#boot-disks). |
| `agent_timeout` | `vm:boot` opts | number (seconds) | How long to wait for the agent on this boot, overriding the profile's [`agent_boot_timeout`](~provium/configuration/provium-toml#agent-boot-timeout). |
...
Each entry is written into the guest's root filesystem before its init
runs, with the parent directories it needs. A path that already exists in
the image is replaced, so a test can substitute a configuration file, a
script, or a whole hook set without rebuilding the profile.

They arrive as one more cpio archive appended to the initrd, which is how
the kernel is willing to receive them: an initrd is a sequence of
archives unpacked in order into the same rootfs, and the last writer of a
path wins. Injected files are appended after the agent overlay, so they
win over both the profile's initrd and the agent's.

Two consequences follow from that mechanism, and neither is a limitation
worth working around:

- **They land in the initrd's rootfs**, not in whatever filesystem the
  guest's init later mounts as its real root. For a profile whose initrd
  is the whole system this is the same thing. For one that pivots to a
  disk or a squashfs, an injected file is what the *initramfs* stage
  sees, which is usually exactly the point — it is how you hand an early
  boot stage a different configuration.
- **They are placed before any userspace runs**, so a guest cannot race
  them, and equally they cannot depend on anything a guest process does.

Paths are taken relative to the root whether or not they carry a leading
slash. A path containing `.` or `..` is rejected when the boot options are
parsed, with the offending path named.

### Boot disks

Each entry becomes a `virtio-blk-pci` device present from the guest's
first instruction, so the firmware and the initramfs can both see it —
which is the difference between these and
[`vm:attach_disk`](#vm-attach-disk-opts), whose attachment is created
after the guest is already running.

Boot disks are attached **after** the profile's
[`disks`](~provium/configuration/provium-toml#disks), so the profile's
keep their device names: a test's first disk is `/dev/vdb` on a profile
that already attaches a medium at `/dev/vda`. Ids share one namespace
with the profile's, so a boot disk reusing a profile id fails the launch
rather than silently displacing it.

Provium never creates the backing file. That is deliberate: a test that
wants a blank disk decides its size, and one that wants a damaged
filesystem builds the damage. Point `path` at a file that already
exists, or the launch fails naming the disk and the path.

Once booted, `vm:disk(id)` returns a [Disk](~provium/reference/disk)
handle for any disk attached this way, for sector access and fault
injection.
...
vm:boot({
    disks = {
        {path = "/tmp/blank.img", id = "target"},
    },
})
vm:disk("target"):fault_inject("eio_write")
```
...
Boots the guest. Returns `self` so calls chain (`provium:vm("a", "peios"):boot()`). The optional `opts` table (`kernel_cmdline`, `kernel_cmdline_append`, `rng_seed`, `initial_time`, `files`, `disks`, `agent_timeout`) merges per field into the VM's pending boot options. Errors if already booted. Under the dispatcher the boot reserves the VM's memory (plus 100 MiB of VMM overhead) and vCPUs first — from the file's [claim](~provium/reference/lab#resource-claims) when it has one, otherwise from the pool — and errors with `cannot boot: …` when the boot is past the claim, or when reserving it from the pool would [deadlock the run](~provium/running-tests/pools-and-parallelism#deadlock).

Boots that are *expected* to fail are asserted through the error this raises. A guest whose init halts, or never starts, never reaches the agent, so `vm:boot` fails once the agent timeout lapses — and its message carries the tail of the guest console, which is the only record such a boot leaves. Wrap the call in `pcall` and assert on that text, and set `agent_timeout` (or the profile's `agent_boot_timeout`) low enough that waiting for it is not the bulk of the run.
...
Wakes a guest that suspended itself — one that wrote `mem` to `/sys/power/state` and is sitting in ACPI S3 with its vCPUs halted. Sends QMP `system_wakeup`; the guest resumes where it stopped. Errors if the VM is not in `Booted` (a suspended guest is still `Booted` from the host's side: its vCPUs stopped, not its VMM), and errors if the guest is not actually suspended — QEMU refuses to wake a running guest — which makes a retried `wakeup` the natural way to wait for a suspend to complete.

The suspend itself is the guest's own act, and the syscall that performs it does not return until the wake — issue it through `vm:syscall_async` or a worker, wake from the host, then await it:

```lua
local pending = worker:syscall_async(sys.NR.write, ...)   -- "mem" → /sys/power/state
wait_until(function() return pcall(vm.wakeup, vm) end, {timeout = 5})
pending:await()
```
...
The same `bufs` / `ptrs` / `nested` options are available on [`worker:syscall`](~provium/reference/worker#worker-syscall-nr).
...
Records a host-side disk attachment and returns a [Disk](~provium/reference/disk) userdata for sector access and fault injection against its backing file. **Nothing reaches the guest**: no device is added to the running QEMU. A disk the guest is meant to see is declared before boot, in the profile's `disks` or in [`vm:boot({disks = …})`](#boot-disks), and then found with `vm:disk(id)`. Opts: `{id=string, size=int, image=string}`. Defaults: `id` = `attached-<vm_name>`, `size` = 4 GiB, `image` = no backing file.
...
CIDs are unique across provium processes running side by side on one host: each process counts from a start scattered by its pid, and a CID is reserved by creating its per-VM scratch directory (`/tmp/provium-qemu/vm-<cid>`) atomically before QEMU launches, so two processes can never hand one guest CID to two VMs.
