...
title: Lab
type: reference
description: The Lab userdata — VMs, bridges, and sub-labs, plus claims, barriers, whole-lab snapshots, and fixtures. The provium global is the root Lab.
related:
...
- `lab:vm("name", "profile", opts)` — create with sizing opts (`memory`, `cpus`). See [VM boot opts](~provium/reference/vm#boot-opts) for the split between creation-time and boot-time keys.
...
Files pushed into the guest via [`vm:push_file`](~provium/reference/vm#vm-push-file-host-path-guest-path-opts) already track themselves — `depends_on_file` is for files the fixture reads on the host side.
...
Remove a resource. Accepts a bare name (tries vm → bridge → sub-lab in order) or a userdata. Removal is [graph-state only](~provium/writing-tests/bridges-and-impairments#declared-vs-realised) — the underlying VM, bridge, or sub-lab is NOT shut down or unrealised.
...
A second `:claim` call on the same file errors — `lab claim already held; one-shot per lab`. The claim is per file, not per lab: one made from a sub-lab or from inside a test is the file's claim. When no pool is wired (REPL / single-file ad-hoc runs) the call still records the one-shot constraint but is otherwise a no-op.

The call blocks until the pool can afford the claim, and errors with `lab claim exceeds total pool budget` if it never could. Time spent blocked here is not charged to the file's timeout.

Once held, the claim is the file's VM budget: every `vm:boot()` and `lab:boot()` in the file takes its memory (plus 100 MiB of VMM overhead per VM) and vCPUs out of the claim, gives them back on shutdown, and never reserves from the pool. A boot the claim cannot cover errors — `vm `x`: cannot boot: needs … but the file's claim is … with … already in use; raise provium:claim to the file's peak` — rather than waiting. Without a claim, each boot reserves from the pool on its own; see [pools and parallelism](~provium/running-tests/pools-and-parallelism#deadlock) for what that costs.
