...
type: concept
description: A persistent watch on a key or subtree reports changes to effective state — best-effort delivery, so a watcher must be ready to re-read.
related:
...
Put watches together with [reject-or-keep](~peios/registry-concepts/configuration-and-meaning) and you get the pattern that runs throughout Peios. A subsystem watches its own configuration subtree; when a value changes, the watch fires; the subsystem re-reads, re-validates, and either applies the new value or keeps its last known-good one and logs the rejection. KMES does this for its tuning; peinit does it for service definitions; the registry itself does it for its own parameters (see [How the registry boots and configures itself](~peios/registry-administration/bootstrap-and-self-configuration)). Configuration is something you *react to*, and watches are the mechanism.
...
If you want the registry's own use of watches — how it picks up changes to its own configuration without restarting — read [How the registry boots and configures itself](~peios/registry-administration/bootstrap-and-self-configuration).

If you want the validation half of the reaction loop — why a changed value might be read and then refused — read [Configuration, not storage](~peios/registry-concepts/configuration-and-meaning).

If you want what it takes to be allowed to watch a key, read [Access control on keys](~peios/registry-security/access-control).
