...
type: concept
description: Layering makes configuration removable — delete a layer and its writes vanish, the next-best write resurfaces, and nothing is left behind.
related:
...
[Layers](~peios/registry-layers/layers) explained how the registry resolves competing writes. This page is the reason it bothers. The layered model buys one thing above all: **configuration you can add and remove as a unit, with revert that is automatic and leaves nothing behind.**
...
A layer makes it with a **tombstone** — a write whose meaning is "no value here". It enters the same per-value contest as any other write ([Layers](~peios/registry-layers/layers)); if it wins, a read of that value returns "not found" rather than falling through to some other layer's write. Like any write, it belongs to a layer — so when that layer is removed, the tombstone goes with it and whatever it was suppressing comes back.
...
- **Not access control.** A layer does not decide who may read or change a value; the [security descriptor](~peios/registry-security/access-control) on each key does. And the two do not mix: a security change made while a layer existed is **not** reverted when the layer is deleted. Security is operational state, not configuration overlay — [Access control on keys](~peios/registry-security/access-control) is where that distinction lives.
...
If you want what "deleting a key" really does once names are layered — and why there is no recursive delete — read [Deleting keys and values](~peios/registry-layers/deleting-keys-and-values).

If you want the security model and its sharp interaction with layers — why deleting a layer reverts its values but *not* a security change made under it — read [Access control on keys](~peios/registry-security/access-control).

If you want to see how a watcher experiences a layer being added or removed — it sees the effective values change, with the layer machinery invisible — read [Watching for changes](~peios/registry-concepts/watches).

For the sandboxing case — per-thread *private* layers that only one caller sees — read [Private hives and layers](~peios/registry-advanced/private-hives-and-layers).
