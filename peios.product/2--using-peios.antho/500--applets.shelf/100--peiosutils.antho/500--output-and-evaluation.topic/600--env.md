...
`-S` follows GNU's quoting rules. Inside single quotes only `\\` and `\'` are escapes; any other backslash is kept literally along with the character after it. Inside double quotes and outside quotes, an unknown backslash sequence is an error.
