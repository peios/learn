...
When `dd` **creates** the output file — an `of=` file that does not already exist — the new file needs a [security descriptor](~peios/security-descriptors/overview), and it inherits one from the directory it is created in.
...
| `1` | The copy failed — a bad operand, an I/O error, a file that could not be opened, or a regular output file that could not be truncated to its final length. |
