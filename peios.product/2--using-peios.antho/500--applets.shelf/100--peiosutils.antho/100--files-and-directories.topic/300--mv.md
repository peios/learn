...
The same applies to POSIX ownership: the copy is a new file owned by whoever ran `mv`, and the source's uid and gid are not carried. Because of that, a cross-file-system move also drops the setuid and setgid bits from the copy, as GNU `mv` does: a set-id file that changed hands would grant the mover's identity, not the original owner's. Data, timestamps and ordinary extended attributes do travel with the file.

When a directory is moved across file systems onto an existing destination, `mv` removes the destination and recreates it. If something else appears under that name in between, `mv` refuses with "destination was replaced while moving" rather than copying into it.
...
A backup is refused when making it would destroy the source: if the source *is* the file the destination's backup would be written over (for example `mv --backup=simple a~ a`), `mv` stops with an error. The check compares file identity, not how the paths are spelled.
