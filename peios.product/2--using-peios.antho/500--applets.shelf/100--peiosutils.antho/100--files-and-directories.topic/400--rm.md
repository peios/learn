...
The failsafe recognises the root by identity, not by spelling: a symbolic link to `/`, a path such as `/..` that resolves to it, and a bind mount of the root file system are all refused.

A recursive removal also refuses to be redirected while it runs. `rm` descends through directory handles rather than paths, never follows a symbolic link it meets on the way down, and if the operand it started from is replaced by something else mid-way it stops with "File changed while removing".
