...
type: concept
description: How a request becomes a concrete plan — dependencies, conflicts, provides and replaces, version choice, and the elevated-authorisation prompt.
related:
...
For the flow that presents and applies the plans resolution produces, read [Installing and removing packages](~peios/package-management/installing-and-removing).

For the single-holder extension of provides, read [Claims](~peios/package-management/claims).

For the repository priorities and trust states resolution consults, read [Repositories and trust](~peios/package-management/repositories-and-trust).
