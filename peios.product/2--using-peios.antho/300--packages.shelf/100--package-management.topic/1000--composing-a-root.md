...
type: reference
description: peipkg-compose builds a fresh, package-owned root from a declarative manifest — offline and deterministically. The lock and build verbs and the lock file.
related:
...
A repository table has the same shape as a [`.repo` file](~peios/package-management/repositories-and-trust). These entries drive metadata fetch and trust verification during the build, **and** are written verbatim to `lcl/conf/peipkg/<name>.repo`, so the booted system inherits the same repositories and the same trust anchors.
...
- **Not the producer side.** It does not build or sign `.peipkg` files and it does not serve repositories — building and signing is [pekit](~pekit/getting-started/what-is-pekit)'s job, and publishing is [`peipkg-repo`](~peios/peipkg/the-tools/peipkg-repo)'s. Compose consumes ordinary peipkg packages and repositories unchanged, exactly as [PSPU §5](~peios/package-format-and-repository-protocol/scope-and-roles) defines them.
