...
type: reference
description: kacs_set_mount_policy and kacs_get_mount_policy, the generation counter behind lazy invalidation, and the SeTcbPrivilege requirement.
related:
...
- [Policy classes](~peios/mount-policies/policy-classes) — what each settable class does.
- [mount](~peios/mount-policies/mount) — setting the policy from the command line at attach time.
- [Privileges](~peios/privileges/overview) — the privilege model behind the SeTcbPrivilege gate.
