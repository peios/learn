...
8. **Infrastructure setup** — create the [control socket](~peios/services-and-jobs/controlling-services), open the JFS device for [ad-hoc jobs](~peios/services-and-jobs/jobs-and-operations), bring up the loopback interface. Control-socket failure → Recovery; the other two are logged as warnings and boot continues.
