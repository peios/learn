...
| `BindsToRecovery` | A [bound](~peios/services-and-jobs/dependencies) target returned to Active; the dependent is auto-restarted. Does **not** consume the restart budget. |
...
| `ConflictEviction` | A [conflicting](~peios/services-and-jobs/dependencies) service started and won. |
...
The grouping matters for what peinit does next: the diagnostic causes are mostly **restart-eligible** (peinit consults [restart policy](~peios/services-and-jobs/supervision)), whereas the definition/graph causes are **never restarted** — retrying a broken definition cannot help. The distinction is spelled out in [Keeping services running](~peios/services-and-jobs/supervision).
...
4. A service's *securable identity* (its [ServiceSecurity](~peios/services-and-jobs/who-can-manage-a-service) descriptor — who can manage it) is independent of its *process identity* (its [token](~peios/services-and-jobs/identity-and-privileges) — what it can access).
5. Readiness is **per start generation** — a `READY=1` from a previous incarnation is never honoured for the current start.

## Where to start

To understand what happens *after* a service crashes — restart policy, backoff, health checks, watchdogs — read [Keeping services running](~peios/services-and-jobs/supervision).

To understand how dependents block on and are released by these states, read [Dependencies and ordering](~peios/services-and-jobs/dependencies).

To see which commands are valid in which state, read the command × state matrix in [Controlling services](~peios/services-and-jobs/controlling-services).
