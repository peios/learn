...
related:
  ...
  - peios/registry-sources/serving-requests
---

Every source starts by registering. You tell the kernel which hives your process backs, and it hands you a source fd to serve on. The full detail is in [`rsi/source.h`](~peios/sdk-rsi-source/rsi-source-h-becoming-a-source); this guide walks the decisions.
...
The returned fd *is* your serve endpoint — you `read(2)` requests and `write(2)` responses on it directly (via the [request](~peios/sdk-rsi-request/rsi-request-h-decoding-requests) and [response](~peios/sdk-rsi-response/rsi-response-h-building-responses) helpers). Closing it deregisters the source and signals EOF to the kernel. Keep it open for the life of the source, and move on to the [serve loop](~peios/registry-sources/serving-requests).

## Next

- **[Serving requests](~peios/registry-sources/serving-requests)** — the loop that runs on the source fd.
- **[`rsi/source.h` reference](~peios/sdk-rsi-source/rsi-source-h-becoming-a-source)** — every field and error.
