...
than an unknown trigger type to be ignored. `tty` carries the same rule:
the bare word names no event, and `tty:` accepts only `released`.
`timer` with no schedule is malformed, as is `timer:` with an empty one.
...
carrying it without a `TTYPath` is refused rather than ignored — it
describes a service that would silently never start, and starting is the
entire point of the trigger. `TTYPrecedence` is refused on the same
grounds. Emptying `TTYPath` (§3.3) takes both down with it.
...
timer, and not any trigger type added later. A disabled service's timers are neither armed
nor serviced.
