...
of each flow. [*net.socket.stamp-on-inet-sockets] AF_UNIX sockets carry the peer-identity machinery of
§3.5 instead and are not stamped.
...
GUID (§3.3.2), the thread-group id, and the task's `comm`. [*net.socket.stamp-contents] Choosing the
...
last one governs: [*net.socket.last-act-governs]

| Act | Where |
|---|---|
| creation | `socket_post_create`; a kernel socket (`sock_create_kern`) is stamped as the **kernel's**, with no token [*net.socket.kernel-socket-stamp] |
| `bind(2)` | after the port reservation permits it (§3.12.1) |
| `listen(2)` | `socket_listen` |
| `connect(2)` | `socket_connect` |
| `accept(2)` | `sk_clone_security`: the child inherits the listener's stamp, its own reference [*net.socket.accept-inherits] |
...
one's own identity. [*net.socket.restamp-self-gated] Sockets that never pass through these hooks (a task
with no token in early boot) read as *unstamped*, which the engine
treats as the kernel's and confesses. [*net.socket.unstamped-reads-kernel]
