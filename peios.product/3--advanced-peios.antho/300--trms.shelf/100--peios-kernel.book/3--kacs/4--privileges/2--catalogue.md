...
ours. [*priv.bits.luid-positions-and-downward-allocation]
...
components — authd and peinit — carry it. [*priv.catalogue.se-create-token] `SeImpersonatePrivilege`
...
checked in exactly one place, the impersonation identity gate (§3.5.2). [*priv.catalogue.se-impersonate]

`SeAssignPrimaryTokenPrivilege` gates installing a token as a
process's primary identity. [*priv.catalogue.se-assign-primary-token] Installation is **self-directed**:
...
`SeTcbPrivilege` (§3.2.3). The privilege is also consulted as a *deny* gate on
...
supplying a SACL at object creation. [*priv.catalogue.se-security]
...
enforcement point at all — it exists purely inside AccessCheck. [*priv.catalogue.se-take-ownership]
...
on the registry key backup and restore paths. [*priv.catalogue.backup-restore-standalone-gates]
...
at-or-below-own-level restriction when a label is written. [*priv.catalogue.se-relabel]
...
directory. [*priv.catalogue.se-change-notify] The bypass has one exception the name does not suggest: it
...
holds the privilege. [*priv.change-notify.chdir-exception] The privilege additionally gates
`open_by_handle_at`, which has nothing to do with traversal. [*priv.change-notify.open-by-handle-at] It is
...
per path walk. [*priv.change-notify.checked-per-directory]

`SeCreateSymbolicLinkPrivilege` creates symbolic links, and is
required in addition to `FILE_ADD_FILE` on the parent directory. [*priv.catalogue.se-create-symbolic-link]
...
| `SeManageVolumePrivilege` | 28 | 0x10000000 | Kernel standalone [*priv.catalogue.se-manage-volume] |
...
token at full access. [*priv.catalogue.se-tcb]
...
process regardless of its descriptor, and does not bypass PIP (§3.7). [*priv.catalogue.se-debug]

- `SeSystemtimePrivilege` changes the clock, [*priv.catalogue.se-systemtime]
- `SeIncreaseBasePriorityPrivilege` raises scheduling priority and
  sets CPU affinity for other processes, [*priv.catalogue.se-increase-base-priority]
- `SeIncreaseQuotaPrivilege` overrides resource limits, [*priv.catalogue.se-increase-quota]
- `SeLockMemoryPrivilege` locks pages in physical memory, [*priv.catalogue.se-lock-memory]
- and `SeAuditPrivilege` writes events to the audit log — it is what
  KMES requires for userspace event emission. [*priv.catalogue.se-audit]

`SeProfileSingleProcessPrivilege` attaches `perf_event_open()` to a
specific other process. [*priv.catalogue.se-profile-single-process] It respects PIP dominance, and own-task
profiling requires nothing. [*priv.profile.pip-dominance-and-own-task] `SeSystemProfilePrivilege` covers
...
system-wide samples include PIP-protected tasks. [*priv.catalogue.se-system-profile] It is an
...
marked used. [*priv.perfmon.satisfied-by-any-of-three] The two profiling privileges are otherwise disjoint
tiers.
...
not a privilege test; see [Port reservations](~peios/network-objects/port-reservations). [*priv.network.no-port-privilege]
...
tree either, so at present nothing anywhere enforces them. [*priv.application-level.none-enforced-anywhere]
...
defined in the kernel and none has an enforcement point. [*priv.reserved.no-enforcement-point]
...
definition exists for it and nothing consults it. [*priv.create-job.bit-62-unused] Submitting a
...
that consults it. [*priv.create-job.present-in-system-token]
...
without error, and are simply inert. [*priv.bits.unallocated-accepted-and-inert]
...
to Everyone. [*priv.default.change-notify-to-everyone] `SeCreateSymbolicLinkPrivilege` is **not** granted by
...
it. [*priv.default.symlink-not-granted] Either can be removed from a specific token by FilterToken.
