...
list is non-empty, and AccessCheck evaluates the DACL twice. [*check.restricted.second-pass-trigger]
...
are invisible to it. [*check.restricted.only-restricting-sids-match] Conditional membership operators — `Member_of`,
...
injected from that list. [*check.restricted.conditional-membership-restricted]
...
conditional membership. [*check.restricted.presence-based] This matches Windows restricted-token
behaviour, where restricting SIDs are always enabled for access
checks.

Access is granted only for rights **both passes agree on** — the
intersection. [*check.restricted.intersection] The restricting list acts as a ceiling: the principal
can never receive more access than the restricting SIDs would
independently justify.

## Write-restricted tokens [*check.restricted.write-restricted]
...
## Privileges bypass the restricted pass [*check.restricted.privileges-bypass]
...
take-ownership grant. [*check.restricted.restored-set]
...
not a restricting SID, no implicit rights are granted. [*check.restricted.owner-implicit]
...
is. [*check.restricted.virtual-groups] This keeps the restricted pass consistent in its handling of these
...
device groups. [*check.restricted.device-groups-swapped]
