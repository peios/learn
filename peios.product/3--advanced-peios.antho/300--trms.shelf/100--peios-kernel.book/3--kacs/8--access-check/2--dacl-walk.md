...
bit. [*check.dacl.first-writer-wins]
...
untouched. [*check.dacl.allow-grants-undecided] A **deny** ACE whose SID matches denies its
not-yet-decided rights — marking them decided but not granted — and
likewise leaves decided bits alone. [*check.dacl.deny-marks-decided]
...
match. [*check.dacl.allow-enabled-groups-only] For **deny** ACEs, both enabled groups and deny-only groups
match: a deny-only group always participates in deny matching whatever
its enabled state. [*check.dacl.deny-matches-deny-only] A group with neither `SE_GROUP_ENABLED` nor
`SE_GROUP_USE_FOR_DENY_ONLY` participates in no matching at all. [*check.dacl.inert-group]

The user SID follows the same rule — when `user_deny_only` is set on
the token, it matches deny ACEs and not allow ACEs. [*check.dacl.user-deny-only]

## Skipping and mapping

ACEs carrying `INHERIT_ONLY` exist purely to propagate to child
objects and are skipped by the walk. [*check.dacl.inherit-only-skipped]
...
request. [*check.dacl.ace-mask-mapped] The mapping works on a local copy — the ACE itself is never
...
right not already decided by an earlier pipeline stage is granted. [*check.dacl.absent-grants-valid-rights] The
valid rights are bounded by `MapGenericBits(GENERIC_ALL, mapping)`
rather than by a raw `0xFFFFFFFF`. [*check.dacl.absent-bounded-by-generic-all]

If the DACL is present but holds zero ACEs, the walk grants nothing. [*check.dacl.empty-grants-nothing]
...
`WRITE_DAC` whatever the DACL says. [*check.dacl.owner-implicit-rights] These are granted **before** the
...
override them. [*check.dacl.owner-implicit-before-walk]
...
(`S-1-3-4`). [*check.dacl.owner-rights-suppresses-implicit] This is a pre-scan performed at the start of
`EvaluateDACL`, before the main loop, and it checks only for the SID's
presence — it does not evaluate any conditional expression on the ACE. [*check.dacl.owner-rights-prescan-presence-only]

During the walk proper, `S-1-3-4` is treated as an ordinary SID
matching the owner, at both allow and deny polarity. [*check.dacl.owner-rights-ordinary-sid] It obeys the same
...
owner does not receive `WRITE_DAC` through this route. [*check.dacl.owner-implicit-bounded-by-decided]
...
begins. [*check.dacl.max-allowed-stripped]
...
being filtered to the requested bits. [*check.dacl.max-allowed-returns-accumulated]
...
always succeeds. [*check.dacl.max-allowed-alone-succeeds]
...
does to targeted ones. [*check.dacl.max-allowed-first-writer-wins] MS-DTYP treats the two differently; KACS does
