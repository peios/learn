...
|---|---|---|
| `process_guid` | UUID | 128-bit identifier for this process instance, generated by the kernel at fork and immutable for the process's lifetime. It is **not** copied from the parent — every process receives a new one. Used by KMES and kernel-internal consumers for identity stamping and event correlation. [*psb.guid.new-and-immutable] |

The process GUID is distinct from the PID. PIDs are recycled; process
GUIDs are unique within a boot, and globally unique in practice. [*psb.guid.unique-within-boot] It is
...
Protected/8192. [*psb.pip.only-protected-tcb] Neither None nor Isolated is defined as a named
constant in the public ABI at all. [*psb.pip.none-isolated-absent-from-abi] The parent process cannot influence the
determination at all — even a compromised peinit running as SYSTEM
cannot forge PIP protection for an unsigned binary. [*psb.pip.parent-cannot-influence] The public
...
| Field | Description |
|---|---|
| `lsv` | **Library Signature Verification.** Only signed shared libraries load. When the process has `pip_type != None` the library's trust level has to be at or above the process's PIP trust; when `pip_type = None` any valid signature suffices and trust is not compared. [*psb.lsv.trust-floor] |
| `wxp` | **Write-XOR-Execute Protection.** No page is simultaneously writable and executable; W+X mappings and transitions between writable and executable are rejected. [*psb.wxp.no-write-and-execute] |
| `tlp` | **Trusted Library Paths.** Shared libraries load only from approved directory prefixes. Weaker than LSV, since it trusts the path rather than the binary. [*psb.tlp.approved-prefixes-only] |
...
| `cfib` | **Backward-Edge Control Flow Integrity.** The hardware shadow stack, Intel CET, is locked on and cannot be disabled by the process. [*psb.cfib.shadow-stack-locked] |
| `pie` | **Position-Independent Executable Requirement.** Non-PIE binaries are rejected at exec, so that ASLR is actually effective. [*psb.pie.non-pie-rejected-at-exec] |
...
invariant. [*psb.mitigations.activation-backed] If any requested mitigation cannot be activated or
verified, the whole operation fails closed without mutating any bit
from that request. [*psb.mitigations.all-or-nothing] Once committed, later operations that would disable
...
already committed. [*psb.mitigations.recommit-never-weakens]
...
or otherwise already violates the invariant in a way KACS can observe. [*psb.wxp.enable-fails-on-existing-wx]
...
TLP-denied. [*psb.tlp.enable-fails-on-denied-mapping] Enabling `lsv` fails if the process already has a
file-backed executable mapping whose signing material is missing,
invalid, or below the required PIP trust. [*psb.lsv.enable-fails-on-untrusted-mapping] Anonymous executable
mappings are governed by `wxp` alone; `tlp` and `lsv` apply only to
file-backed ones. [*psb.mitigations.anonymous-exec-is-wxp-only]
...
the protection true for the target. [*psb.mitigations.arch-fails-closed]
...
alone, with nothing to enable. [*psb.sml.not-affected-satisfies]
...
every request. [*psb.cfif.enodev-always] `cfib` works, through shadow-stack enable-and-lock,
with one restriction: enabling it on a task other than the caller
fails. [*psb.cfib.self-only]
...
process creation. [*psb.mitigations.event-gated] They still follow the one-way commit rule, and have
...
## UI access (one-way) [*psb.ui-access.set-once-fixed]
...
| Field | Description |
|---|---|
| `no_child_process` | Once set, the process creates no child processes — fork, or clone without `CLONE_THREAD`. New threads are unaffected, and the flag is never cleared. [*psb.no-child-process.blocks-fork] |
...
it has finished spawning its own workers, for instance. [*psb.no-child-process.two-set-points]
...
entries of at most 4096 bytes each. [*psb.tlp.cache-bounds] Every prefix begins with `/` and
**ends** with `/` — so that `/usr/lib/` does not match `/usr/libevil`
— and contains no embedded NUL byte. [*psb.tlp.prefix-slash-terminated] An empty, relative,
...
written. [*psb.tlp.invalid-prefix-rejected-atomically]
...
fills it. [*psb.tlp.no-production-writer] In a shipping build the cache is therefore permanently
...
attempts. [*psb.tlp.empty-cache-denies-all]
