...
set or tightened but never cleared or loosened. [*token.mutability-classes]
...
| `user_deny_only` | bool | When true, the user SID matches only deny ACEs, never allow ACEs. Set at creation by CreateToken or FilterToken. True whenever `write_restricted` is true. [*token.user-deny-only-with-write-restricted] |
...
carrying `SE_GROUP_LOGON_ID`, which is where AccessCheck finds it. [*token.logon-sid-materialised-in-groups]

The group SID set is fixed at creation — adjustment never adds or
removes a SID. [*token.group-set-fixed] Individual groups can be enabled or disabled by
...
(`SE_GROUP_USE_FOR_DENY_ONLY`) cannot be re-enabled. [*token.group-toggle-limits]

A token holds at most 1024 group entries including the kernel-injected
logon SID, so CreateToken accepts at most 1023 caller-supplied groups. [*token.group-limit-1024]
...
| Field | Type | Description |
|---|---|---|
| `integrity_level` | uint | Numeric integrity level. Standard values are 0 (Untrusted), 4096 (Low), 8192 (Medium), 12288 (High), 16384 (System), but any unsigned integer is valid and is compared numerically against object labels. [*token.integrity-level-any-unsigned] |
...
constraints at runtime, neither loosening nor tightening them. [*token.mandatory-policy-immutable] This is
...
creation. [*token.priv.no-add-after-creation] **Enabled** — the privilege is currently active; only
present privileges can be enabled or disabled. [*token.priv.enable-requires-present] **Enabled by default**
— the creation-time enabled state, which adjustment can restore.
**Used** — the privilege has been exercised during this token's
lifetime; monotonic, and never cleared once set. [*token.priv.used-is-monotonic]
...
states together. [*token.priv.removal-clears-three-states]
...
A token is created as Default. [*token.elevation-default-at-creation] Only `KACS_IOC_LINK_TOKENS` sets Full
or Limited, when a linked pair is established, and once set neither
reverts to Default on that token object. [*token.elevation-only-link-sets] The role is sticky: relinking
can replace the partner but never converts Full to Limited or the
reverse. [*token.elevation-role-sticky] DuplicateToken and FilterToken produce new token objects
whose `elevation_type` starts again at Default, because a new token is
not part of any linked pair. [*token.elevation-resets-on-derivation]
...
| Field | Description |
|---|---|
| `owner_sid_index` | Index into `[user_sid, groups...]` selecting the default owner SID for new objects: 0 is the user SID, 1..N are `groups[0..N-1]`. The referenced SID is the user SID or a group carrying `SE_GROUP_OWNER`. [*token.owner-index-rule] |
| `primary_group_index` | Index into `[user_sid, groups...]` selecting the default primary group. The referenced SID is the user SID or any group SID on the token. [*token.primary-group-index-rule] |
...
| `created_at` | timestamp | When the original token was minted by CreateToken. Copied unchanged by DuplicateToken, FilterToken, and `NEW_PROCESS_MIN`, so it tracks original minting rather than duplication. [*token.created-at-tracks-minting] |
| `expiration` | timestamp | When the token becomes invalid; zero means no expiry. Not enforced by AccessCheck. [*token.expiration-not-enforced] |
...
| Field | Description |
|---|---|
| `modified_id` | Counter incremented on any token adjustment. [*token.modified-id-bumps-on-adjustment] |
...
that leaves the counter alone. [*token.modified-id-exemptions]
...
query class, and no cache anywhere invalidates on it. [*token.modified-id-nothing-invalidates]

## Interactivity scope (adjustable)

| Field | Description |
|---|---|
| `interactivity_scope` | 0 for services, which have no interactive environment; 1 and above for interactive and remote user environments. Changing it requires `SeTcbPrivilege`. [*token.interactivity-scope-needs-tcb] |
...
can create a token carrying them. [*token.lcs-credentials-fixed-and-gated]
...
| Field | Type | Description |
|---|---|---|
| `confinement_sid` | SID? | Places the token in a default-deny sandbox; null means unconfined. When set, AccessCheck switches to default-deny and access requires an explicit grant to this SID or to a SID present in `confinement_capabilities`. [*token.confinement-sid-default-deny] |
| `confinement_capabilities` | SID_AND_ATTRIBUTES[] | Declared access capabilities for a confined process, empty if none. The `attributes` field is carried for wire-format uniformity only: AccessCheck treats capability membership as presence-based and consults neither `SE_GROUP_ENABLED` nor `SE_GROUP_USE_FOR_DENY_ONLY` when matching confinement SIDs. [*token.confinement-capabilities-presence-based] |
| `isolation_boundary` | bool | Adds namespace filtering on top of confinement, making objects outside the boundary invisible rather than merely denied. Requires `confinement_sid`. Settable at creation but not enforced. [*token.isolation-boundary-not-enforced] |
| `confinement_exempt` | bool | Escape hatch: confinement restrictions are not evaluated at all. [*token.confinement-exempt-skips-evaluation] |
...
present — strict confinement is expressed by the caller omitting it. [*token.all-application-packages-not-synthesised]
...
| Field | Type | Description |
|---|---|---|
| `audit_policy` | u32 | Per-token audit overrides as a bitmask, fixed at creation — no adjustment operation exists. [*token.audit-policy-fixed] |

| Flag | Value | Description |
|---|---|---|
| `OBJECT_ACCESS_SUCCESS` | 0x0001 | Audit successful object access. |
| `OBJECT_ACCESS_FAILURE` | 0x0002 | Audit failed object access. |
| `PRIVILEGE_USE_SUCCESS` | 0x0004 | Audit successful privilege exercises: the privilege contributed requested bits that survive into the final granted result. [*token.audit-policy.privilege-use-success] |
| `PRIVILEGE_USE_FAILURE` | 0x0008 | Audit failed privilege exercises: the privilege contributed requested bits during evaluation, but they do not survive into the final result. [*token.audit-policy.privilege-use-failure] |
...
requires. [*token.audit-policy-additive] It follows impersonation: when service A impersonates client
...
| Field | Description |
|---|---|
| `projected_uid` | Linux UID for the user SID, precomputed by authd when the token is minted (PSPU §2); 65534 for the anonymous identity. [*token.projection.anonymous-is-nobody] |
...
token; KACS never resolves a SID-to-UID mapping at runtime. [*token.projection.precomputed-never-resolved] Projection
reflects all groups regardless of enabled state, so adjusting groups
does not trigger recalculation. [*token.projection.ignores-enabled-state] §3.10 covers how the projection is
used.
...
| Field | Description |
|---|---|
| `security_descriptor` | The token's own SD, controlling who may query, adjust, duplicate, or impersonate it. [*token.own-sd-governs-handle-ops] |
