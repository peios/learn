...
the thread's effective token. [*token.decisions-use-effective-token]

Every live userspace thread has one. [*token.every-thread-has-one] KACS-mediated authorization never
...
a meaningless identity. [*token.null-credential-fails-closed]
...
visible to every thread sharing it. [*token.shared-within-process] At fork the child receives an
...
the parent at fork. [*token.real-cred-is-objective]
...
installed by a server thread acting for a client. [*token.cred-is-subjective]
...
`cred` to `real_cred`. [*token.impersonation-swaps-cred-only]
...
what counts, not the worker flag. [*token.override-creds-authoritative]
...
post-open operations. [*token.cached-handle-authority-persists] User-originated asynchronous work that has
neither captured credentials nor cached handle authority reaches KACS
without a token-bearing credential and fails closed. [*token.uncaptured-async-fails-closed]

Kernel-originated infrastructure work running under the boot SYSTEM
credential evaluates as SYSTEM. [*token.kernel-work-evaluates-as-system] There is no separate worker-flag-based
