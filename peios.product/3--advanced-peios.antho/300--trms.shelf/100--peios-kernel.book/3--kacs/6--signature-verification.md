...
entry. [*sig.key-table.entry-layout]
...
exactly Protected (512) with `PeiosTcb` trust (8192). [*sig.key-table.validation-einval] Both rejections
...
entirely. [*sig.key-table.invalid-disables-all]

The consequence is that a verified binary is always Protected/8192. [*sig.verified-always-protected-tcb]
...
change to the validator, not merely an added key. [*sig.isolated-type-unreachable]
...
invalidates the whole result. [*sig.size-snapshot-revalidated]
...
goes straight to the xattr. [*sig.magic-selects-path]
...
prefix does not match. [*sig.elf.section-name-exact-match]
...
whatever happens next. [*sig.elf.section-commits-path] A wrong section type, a size other than 3310, a
...
falling back. [*sig.elf.section-failures-yield-unsigned] This is deliberate: without it, an attacker could craft
...
section header table or string table lying outside the recorded size. [*sig.elf.structural-failures-commit]
...
with no section headers falls through to the xattr normally. [*sig.elf.no-section-headers-falls-through]

The xattr path reads `security.peios.sig` and requires exactly 3310
bytes; any other size is treated as unsigned. [*sig.xattr.exact-size] The read bypasses the
LSM xattr hooks, so FACS does not mediate the verifier's own read of
the signature. [*sig.xattr.read-bypasses-lsm-hooks]
...
verbatim, along with everything else. [*sig.hash.elf-section-zeroed] For an xattr source the hash
covers the entire file with no exclusions, and that applies to ELF
files reaching the xattr path as well as to non-ELF ones. [*sig.hash.xattr-covers-whole-file] Hashing
...
the table, in order, returning on the first success. [*sig.verify.exhaustive-trial-in-order] There is no key
...
of *which key verified*, never of anything the signer encoded. [*sig.tier-from-key-not-blob]
...
signature made under a non-empty context simply fails to verify. [*sig.empty-fips204-context]
...
unavailable ML-DSA transform, or a key the transform will not accept. [*sig.verifier.tri-state]
...
with `EACCES`. [*sig.unverifiable.exec-eacces]
...
fails closed. [*sig.unverifiable.lsv-denies]
...
matching key all yield None/0. [*sig.exec.tier-from-matched-key]

**Exec proceeds in every case where the question could be answered.** [*sig.exec.proceeds-regardless]
...
process state only from `bprm_committed_creds`. [*sig.exec.stage-then-commit] An exec that fails
between staging and commit leaves the process state untouched, and the
stale pending value is cleared by the next exec or at task teardown. [*sig.exec.failed-exec-leaves-state]
...
last file processed — not from the script. [*sig.exec.shebang-takes-interpreter] A TCB-signed interpreter
...
target file, so a symlink inherits its target's level by construction. [*sig.exec.symlink-resolves-to-target]

Once committed, the values are fixed for the lifetime of the process
image, inherited at fork, and re-derived at the child's exec. [*sig.exec.fixed-and-inherited]
...
committed, and LSV pins on success too, before allowing the mapping. [*sig.pin.on-verified-exec-and-lsv]
...
runs unprotected. [*sig.pin.failure-downgrades-to-none] Under LSV a pin failure denies the mapping instead. [*sig.pin.failure-denies-lsv-mapping]
...
independently of the mode-support test. [*sig.pin.rejects-content-mutation] File ioctls that mutate
content, ranges, or allocation are rejected, and so are ioctls the
kernel cannot classify — unknown ioctls fail closed on a pinned inode. [*sig.pin.unknown-ioctl-fails-closed]
Mutation or removal of the `security.peios.sig` xattr is rejected as
well. [*sig.pin.rejects-sig-xattr-mutation]

The pin is conservative and one-way. It is set once and cleared only
when the inode is allocated or freed, never while the inode is live. [*sig.pin.cleared-only-at-inode-lifecycle]
...
the mapping with `EACCES`. [*sig.lsv.denies-unsigned-eacces] On success the library's tier is compared
...
libraries. [*sig.lsv.image-must-dominate-process] With one key in the table this reduces to "is it signed
with the TCB key?"
...
touch. [*sig.lsv.hashes-whole-file]
...
LSV. [*sig.lsv.mprotect-check-order] Anonymous mappings have neither a path nor a signature, so TLP
and LSV skip them and only WXP applies. [*sig.lsv.anonymous-skips-tlp-lsv]
...
at any other tier counts as no signature. [*sig.firmware.peiostcb-floor]
...
device, and those are what is hashed — no second read, no TOCTOU. [*sig.firmware.post-read-hashes-loaded-bytes] A
...
that path verifies the file through the reader probe instead. [*sig.firmware.partial-read-uses-probe] The
hash covers the compressed bytes as they sit on disk, so verification
precedes decompression. [*sig.firmware.hashes-compressed-bytes]
...
that silently stops probing. [*sig.firmware.default-is-log] Enforcement becomes the default once a
...
is consulted. [*sig.facs.open-denied-before-signing] Signing determines the PIP level of a binary that is
