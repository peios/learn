...
## Core rights [*facs.legacy.core-rights]
...
listing is a compat right. [*facs.legacy.directory-core-excludes-list]
...
core ends up with `FILE_APPEND_DATA` and `FILE_WRITE_DATA` together. [*facs.legacy.append-trunc-order]
...
APIs at all. [*facs.legacy.read-attributes-always-core]

## Compat rights [*facs.legacy.compat-rights]
...
is stamped on the descriptor; otherwise the open fails with `EACCES`. [*facs.legacy.core-missing-eacces]
...
carry no granted mask and are left unmanaged. [*facs.legacy.o-path-unmanaged] They serve as namespace
anchors for the `*at()` syscalls.

`fstat()` and `fstatfs()` on them are allowed unconditionally. [*facs.legacy.o-path-fstat-unconditional]
`fchdir()` runs a live `FILE_TRAVERSE` check at use time. [*facs.legacy.o-path-fchdir-live] `fchmod()`,
`fchown()`, `fgetxattr()`, `fsetxattr()`, `ioctl()` and `mmap()` are
denied with `EBADF` — though `futimens()` currently has no such guard. [*facs.legacy.o-path-ebadf]
...
race-free object identity without snapshot authorization. [*facs.legacy.o-path-live-checks]
