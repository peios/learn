...
descriptor's whole lifetime regardless of any later descriptor change. [*facs.handle.mask-immutable]
...
the granted mask through every path described below. [*facs.handle.audit-mask-stamped]
...
observe the same class. [*facs.handle.policy-per-superblock]

**`unmanaged`** puts the mount outside the handle model entirely: no
granted mask is stamped on its file descriptions. [*facs.handle.unmanaged-no-mask] **`facs_deny_missing`**
...
`facs_synthesize_ephemeral`: ramfs, FAT, exFAT, NFS client mounts,
ISO9660, and cgroup2. StrataFS is fixed at `facs_deny_missing`. Everything else
...
StrataFS's. [*facs.handle.unmanaged-not-settable]
...
- `O_PATH` descriptors, which are not managed;
- NFS client mounts, which are ephemeral-synthesising and retain dual
  authority with the server;
- `/proc`, which is unmanaged and PIP-protected; or
- `/sys`, which is unmanaged and carries a hardcoded rule instead —
  ...
  built-in descriptor. [*facs.handle.sysfs-write-builtin-descriptor]

Descriptors for processes obtained through `pidfd` bypass FACS at open
as well. [*facs.handle.pidfd-bypasses-open]
...
- `dup`/`dup3` and `fork` produce the same open file description and
  therefore the same rights; [*facs.handle.dup-fork-same-rights]
- descriptors without `FD_CLOEXEC` survive exec unchanged; [*facs.handle.exec-preserves-mask]
- `SCM_RIGHTS` transfers the descriptor as a capability token, with
  possession as authorization and no re-check of the receiver's
  identity; and [*facs.handle.scm-rights-capability]
- `pidfd_getfd()` is gated by an AccessCheck for `PROCESS_DUP_HANDLE`
  against the target process, after which the caller receives the
  descriptor with its full mask. [*facs.handle.pidfd-getfd-gate]
...
at open, against the opener. [*facs.handle.mic-pip-at-open] A high-integrity process that opens a
...
opened. [*facs.handle.backing-snapshot-captured] No new AccessCheck runs against the task executing the
stacking filesystem. [*facs.handle.backing-no-new-accesscheck]
...
file — only the values. [*facs.handle.backing-admission-narrow] It is not a general snapshot-cloning
mechanism.
...
inherited authority. [*facs.handle.copy-up-context-precedence] When an exactly-bound copy-up backing file is
...
caller authorization nor emits a second caller audit event. [*facs.handle.no-duplicate-provider-check] The
...
installed. [*facs.handle.mmap-backing-verifies-agreement]
...
mounts, immutable state, quota, space, I/O, or format validation. [*facs.handle.backing-suppresses-kacs-only]
