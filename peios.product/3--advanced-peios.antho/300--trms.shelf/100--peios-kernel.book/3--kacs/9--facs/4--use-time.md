...
live AccessCheck. [*facs.use.mask-check-per-operation]

## Data operations [*facs.use.data-operations]
...
| `mmap PROT_WRITE \| MAP_SHARED` | `FILE_WRITE_DATA` — `FILE_APPEND_DATA` alone is insufficient [*facs.use.mmap-shared-write-needs-write-data] |
| `mmap PROT_WRITE \| MAP_PRIVATE` | `FILE_READ_DATA` — copy-on-write, no write to the file [*facs.use.mmap-private-needs-read] |
...
| `fsync` / `fdatasync` | `SYNCHRONIZE` [*facs.use.fsync-requires-synchronize] |

A `fallocate` mode outside the supported set fails closed, and
`PUNCH_HOLE` additionally requires `KEEP_SIZE`. [*facs.use.fallocate-unsupported-mode-fails]

## Metadata operations [*facs.use.metadata-operations]
...
| `chmod` / `fchmodat` / `fchmod` | `WRITE_DAC` [*facs.use.chmod-requires-write-dac] |
| `chown` / `lchown` / `fchownat` / `fchown` | `WRITE_OWNER` [*facs.use.chown-requires-write-owner] |
...
| `listxattr` / `llistxattr` / `flistxattr` | none [*facs.use.listxattr-needs-no-right] |
...
probe-then-tolerate callers behave sensibly. [*facs.use.posix-acl-write-eopnotsupp]

## Directory traversal [*facs.use.directory-traversal]
...
missing. [*facs.use.change-notify-bypasses-traverse]
...
(§3.4.2). [*facs.use.chdir-chroot-live-check] An ordinary `fchdir()` checks the descriptor's cached mask;
an `O_PATH` `fchdir()` runs live. [*facs.use.fchdir-cached-mask]

## Watch placement [*facs.use.watch-placement]
...
does not bypass it — that privilege covers traversal, not reading. [*facs.use.watch-change-notify-no-bypass]
...
resolves that to `SeTcbPrivilege`, so these are TCB-only. [*facs.use.fanotify-mount-marks-tcb] PIP
...
## Append-only enforcement [*facs.use.append-only-enforcement]
...
operation. [*facs.use.append-intent-definition] Requesting both `RWF_APPEND` and `RWF_NOAPPEND` together
fails with `EACCES`. [*facs.use.append-noappend-conflict-eacces]
...
modes. [*facs.use.append-only-denied-set]

## fcntl [*facs.use.fcntl]
...
reduction. [*facs.use.setfl-clear-append-denied] Adding `O_NOATIME` requires `FILE_WRITE_ATTRIBUTES` and
clearing it is always allowed. [*facs.use.setfl-noatime] Changing only `O_NONBLOCK`, `O_NDELAY`
or `O_DIRECT` needs no KACS right, though ordinary Linux validation
still applies. [*facs.use.setfl-flags-need-no-right]
...
and signal validation still applies. [*facs.use.fcntl-descriptor-local]
...
`F_WRLCK` or `F_UNLCK` values. [*facs.use.fcntl-lock-passthrough] An unknown lock type fails closed
there.

For `F_NOTIFY`, removing a watch — a zero event mask, ignoring
`DN_MULTISHOT` — requires nothing. Installing one with any known `DN_*`
event requires `FILE_LIST_DIRECTORY`. [*facs.use.fcntl-notify-install] Unknown `DN_*` bits on a managed
descriptor fail closed. [*facs.use.fcntl-notify-unknown-bits]

Unmanaged descriptors sit outside the handle check entirely, and an
unknown fcntl command on a managed one fails closed. [*facs.use.unknown-fcntl-fails-closed]

## ioctl [*facs.use.ioctl-classification]
...
`FS_IOC32_SETVERSION` and the compat preallocation commands. [*facs.use.ioctl-compat-aliases]
...
Linux and fops validation still applying. [*facs.use.ioctl-descriptor-local]
...
state, and Linux's own `CAP_SYS_ADMIN` checks still apply on top. [*facs.use.ioctl-freeze-cap-sys-admin]
...
rights as on files. [*facs.use.ioctl-flags-on-directories]
...
Linux's device-specific validation may still deny. [*facs.use.ioctl-device-semantics-out-of-scope]
...
bit set, by way of `capable_wrt_inode_uidgid(CAP_DAC_OVERRIDE)`. [*facs.use.exec-mode-bit-from-linux] The
...
against the re-opened file rather than the cached mask being consulted. [*facs.use.execveat-live-check]
