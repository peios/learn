...
is denied, the open fails. [*facs.open.strict-all-or-nothing] This is the **strict** mode, in contrast to
...
requested set. [*facs.open.maximum-allowed-computes-max] Alone it is invalid, because it defines no file mode,
and fails with `EINVAL`. [*facs.open.maximum-allowed-alone-einval]

## The required data right [*facs.open.required-data-right]
...
can neither read nor write the file's contents. [*facs.open.execute-only-handle]
...
`FMODE_READ`. [*facs.open.dir-list-satisfies-data-right]
...
`EOPNOTSUPP`. [*facs.open.dir-write-aliases-eopnotsupp] Those are parent-directory authorization rights for
...
file GenericMapping for metadata, standard and generic rights. [*facs.open.special-nodes-are-file-objects]

`FILE_EXECUTE` is not a valid data substitute for them — a
`kacs_open` naming it on such a node fails closed. [*facs.open.execute-not-valid-on-special-nodes] The Linux object
...
`AT_SYMLINK_NOFOLLOW` fails with `ELOOP`. [*facs.open.symlink-nofollow-eloop] This differs deliberately
...
## Create dispositions [*facs.open.create-dispositions]
...
the parent. [*facs.open.supersede-rights] The new file gets a new inode and a new descriptor,
...
still reference it. [*facs.open.supersede-new-inode]

`FILE_OVERWRITE` truncates in place — same inode, same descriptor,
hardlinks preserved — and requires `FILE_WRITE_DATA`. [*facs.open.overwrite-in-place]
...
failing; the creating dispositions fail with `EOPNOTSUPP`. [*facs.open.unmanaged-create-eopnotsupp]

### The DELETE fallback [*facs.open.delete-fallback]
...
|---|---|---|
| 0x0001 | `KACS_CREATE_OPT_DIRECTORY` | The target has to be a directory. When creating, create a directory rather than a regular file; when opening an existing non-directory, fail with `ENOTDIR`. [*facs.open.create-opt-directory] |
...
bit fails with `EINVAL`. [*facs.open.reserved-create-option-einval]
...
because they preserve the description. [*facs.open.delete-on-close-lineage] Once a lineage exists for a
file object, later opens of that object fail closed rather than
emulating share-mode compatibility. [*facs.open.delete-on-close-no-share] The unlink happens at final close
of that lineage — not at open, and not at generic inode
last-reference drop — and if the pathname is already gone by then the
close path treats it as a no-op rather than a new error. [*facs.open.delete-on-close-final-close] Regular files
...
inheritance algorithm. [*facs.open.null-descriptor-inherits]
...
descriptor. [*facs.open.supplied-on-existing-einval] `FILE_OPEN` with a descriptor supplied fails with
`EOPNOTSUPP`. [*facs.open.supplied-with-file-open-eopnotsupp]
...
descriptor for the requested access. [*facs.open.check-against-new-descriptor] Parent create rights authorize
...
created file or directory is removed — and the syscall fails. [*facs.open.failed-check-rolls-back]
...
case any owner is allowed. [*facs.open.supplied-owner-constraint] A supplied SACL is treated as a full SACL
input rather than a label-only fragment, and therefore requires
`SeSecurityPrivilege`. [*facs.open.supplied-sacl-requires-privilege] If that SACL carries an explicit mandatory
...
`SeRelabelPrivilege` is held. [*facs.open.supplied-label-constraint]
...
for directories. [*facs.open.fixed-create-modes] These are compatibility metadata only — and if Linux
...
symlinks. [*facs.open.no-native-special-creation] Those stay on the Linux namespace APIs — `mknod()`,
`mkfifo()`, Unix `bind()` to a path — governed by the namespace hooks.
NTFS is excluded from native creation entirely. [*facs.open.ntfs-excluded]
...
and simply creates one reports `KACS_STATUS_CREATED`. [*facs.open.status-superseded-only-if-replaced]
