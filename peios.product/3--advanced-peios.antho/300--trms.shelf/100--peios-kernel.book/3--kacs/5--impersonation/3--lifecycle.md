...
then on — and calls `connect()`. [*imp.capture.level-changeable-bounds-later-captures] The kernel's LSM hook fires on the
...
Identification level unless the listener set its own level. [*imp.capture.listener-identity-to-connecting-end] The
...
socket that is not listening. [*imp.restamp.listening-socket-only] Clients that connect before the restamp
see the previous holder, which is honest: that is who was listening. [*imp.restamp.not-retroactive]
...
never recorded. [*imp.capture.anonymous-records-no-real-identity] At Impersonation or Delegation, the thread's effective
...
is how identity cascades across local services. [*imp.capture.stores-effective-token] At Identification, the
...
moves). [*imp.peer-token.fd-access-mask]
...
credential to `real_cred` and its service identity with it. [*imp.revert.restores-real-cred]

## Anonymous

Any thread may impersonate the Anonymous identity without passing
either gate. [*imp.anonymous.no-gates] Assuming Anonymous is always a downgrade — the token has
...
identity. [*imp.anonymous.socket-constructs-minimal-shape]

## Double impersonation [*imp.double.revert-then-reimpersonate]
...
impersonation safe. [*imp.mic.reads-effective-token] A thread cannot act at Impersonation or Delegation
...
**PIP reads the PSB**, because there is no equivalent ceiling for it. [*imp.pip.reads-psb-not-effective-token]
...
data that end has consumed so far. [*imp.register.connect-capture-is-first-entry] Identity can also travel with the
...
handler can never be told an identity for a request it has not read. [*imp.register.anchored-at-read-position]
`KACS_SO_PEER_TOKEN` returns a fresh fd to an immutable snapshot each
time; two calls may name different tokens, but a token never changes. [*imp.peer-token.immutable-snapshot]
...
- Automatically, with `KACS_SO_PASS_TOKEN` set on the sending end:
  ...
  lower of the end's impersonation level and the token's own. [*imp.pass-token.every-send-carries-effective] The derivation is cached against the
  effective token it was made from, so a run of sends under one
  identity costs one derivation. [*imp.pass-token.derivation-cached] A thread that impersonates client A,
...
- Explicitly, by attaching a token fd in a `KACS_SCM_TOKEN` cmsg. The
  ...
  two-gate model runs with the sender as installer. [*imp.scm-token.gated-as-impersonation] An attach the gates
  would cap below the token's own level fails with `EPERM` — an
  explicit act fails loudly where an install would silently downgrade. [*imp.scm-token.cap-fails-eperm]
...
always a downgrade. [*imp.scm-token.anonymous-attaches-ungated] At most one token per message. [*imp.scm-token.one-per-message]
...
fragments. [*imp.receive.identity-on-every-fragment] A stream read stops at an identity boundary rather than
...
why it is the recommended type for Peios-native protocols. [*imp.receive.read-stops-at-identity-boundary] As the
...
consumption, not where ancillary data is copied out. [*imp.register.advances-on-consumption-only]
...
supplied. [*imp.cmsg.delivered-on-identity-change] A handler that never reads ancillary data still has the
register: `recvmsg`, then `getsockopt(KACS_SO_PEER_TOKEN)`, gives the
identity of what it just read. [*imp.register.getsockopt-after-read] Parsing the cmsg is the path for
...
datagram that carries one and is never retained. [*imp.dgram.no-register]

## Supported transports

Peer-token capture works on two AF_UNIX socket types. [*imp.transport.stream-and-seqpacket] `SOCK_STREAM`
...
is no register. [*imp.dgram.per-message-identity-only]
...
fails with `EOPNOTSUPP`. [*imp.peer-token.eopnotsupp-non-unix] A `socketpair()` end is connected but starts
...
after which the register works as on any connection. [*imp.peer-token.socketpair-enodata] A socket that is
not yet connected fails with `ENOTCONN`. [*imp.peer-token.enotconn] Pipes and FIFOs are not
sockets at all, so `getsockopt` itself fails with `ENOTSOCK` before
KACS is reached. [*imp.peer-token.enotsock-pipes]
