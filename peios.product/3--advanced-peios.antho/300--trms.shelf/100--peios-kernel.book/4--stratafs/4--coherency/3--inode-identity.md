...
stratafs superblock's own anonymous device, not the provider's. [*inode.reported-device-is-mount-device]
...
number handed out is 2. [*inode.first-number-is-2] The provider's own inode number and device are
never read for this purpose. [*inode.number-allocated-not-derived]
...
compares equal too. [*inode.equal-iff-same-provider-object] It does not follow the specification's advice to
...
32-bit build the number truncates. [*inode.number-truncates-on-32-bit]
...
number is not. [*inode.root.number-fixed-attributes-live]
...
paths naming one object disagree. [*inode.root.stale-number-after-provider-change] This is tracked as a defect.

## Attributes of merged directories
...
a real directory's attributes rather than a composite. [*inode.merged-dir.attributes-from-provider]

Its link count is forced to 1, both in the cached inode and in the
reported `stat`. [*inode.merged-dir.nlink-forced-to-1] The true count of subdirectories spans strata and
...
re-associated with a different provider. [*inode.never-rebound-by-resolution]
...
through the descriptor reach the copy. [*inode.copy-up.descriptor-keeps-inode]
...
inode's number, so before any copy-up `fstat` and `stat` agree. [*inode.open.fstat-matches-stat] After
...
long as that descriptor lives. [*inode.copy-up.fstat-diverges-from-stat] §4.8 records it.
...
path reference, and the identity map a third, held until unmount. [*inode.pins-provider-until-unmount]
