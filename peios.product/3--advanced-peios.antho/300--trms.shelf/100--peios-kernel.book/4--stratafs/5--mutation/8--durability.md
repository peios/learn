...
is never forwarded to whichever object currently provides the path. [*durability.fsync-follows-the-descriptor]
...
retired pre-copy-up file (§4.5.7) is not. [*durability.fsync-targets-the-copy]

## Synchronising a merged directory
...
of the call, and fails if any of them fails. [*durability.dirsync-covers-every-stratum] The loop continues past a
failure, so every stratum is still attempted, and the first error is
what is returned. [*durability.dirsync-returns-the-first-error]
...
read from the participant set settled at open (§4.3.4). [*durability.dirsync-evaluated-at-call-time] Both halves of
that matter:
...
unfreeze, freeze-super or thaw-super operation is registered at all. [*durability.freeze-eopnotsupp]
