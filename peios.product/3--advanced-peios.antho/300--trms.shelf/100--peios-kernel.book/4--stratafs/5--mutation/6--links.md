...
1. `P` must accept modification. Otherwise `EXDEV`. [*link.source-provider-must-be-writable]
2. The destination must not be held by any participating stratum.
   Otherwise `EEXIST`. [*link.destination-must-not-be-held]
3. `P` must hold the directory containing the destination. Otherwise
   ...
   whether or not `P` is the create stratum. [*link.destination-parent-in-provider]
4. The link is created in `P`. [*link.created-in-the-provider]
...
A link request is never satisfied by copying the source up. [*link.never-copies-up] The result
...
the link is rolled back; a failed rollback is audited. [*link.rolled-back-on-install-failure]
...
stratum that is absent or does not exist fails with `EROFS`. [*link.unnamed-goes-to-create-stratum]

Linking an unnamed file into a different mount fails with `EXDEV`,
checked both by superblock and by mount, and by the VFS as well. [*link.unnamed-cross-mount-exdev]
...
established by inheritance there. [*link.symlink-created-in-create-stratum]
...
the mount, nor the reverse. [*link.symlink-target-verbatim]
...
root and may re-enter this mount, a different stratafs mount, or none. [*link.symlink-target-resolved-by-vfs]
