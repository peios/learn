...
the provider's stratum, and the unlink is performed there. [*remove.unlink-in-provider]
...
allows an object it provides to be modified in place. [*remove.any-modifiable-stratum]
...
lookup finds the lower entry. [*remove.lower-entry-becomes-visible]
...
`EPERM`. [*remove.immutable-provider-eperm] The outer inode carries the provider's inode flags, so the VFS
...
`ENOTEMPTY`. [*remove.rmdir-merged-emptiness]
...
directory was empty. [*remove.rmdir-access-check-precedes-scan] Without that ordering the emptiness test would be
...
directory that looks empty through the mount. [*remove.rmdir-ignores-staging-entries]
...
there. [*remove.rmdir-recovers-orphans]
...
remaining strata — which, by the emptiness condition, is empty. [*remove.rmdir-reveals-lower-directory]
...
no whiteout machinery exists anywhere in the filesystem. [*remove.touches-only-the-provider]
...
already gone and the deletion is genuinely complete. [*remove.no-success-without-removal]
