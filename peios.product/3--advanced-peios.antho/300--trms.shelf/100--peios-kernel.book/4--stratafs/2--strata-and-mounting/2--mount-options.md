...
byte is stored literally, and the backslash is consumed. [*mount.escape-set]
...
path survives the split rather than being read as an option boundary. [*mount.escaped-comma-survives-split]
...
create stratum. [*mount.read-only-refuses-all-mutation]
...
any remount that supplies `strata=` at all is refused with `EINVAL`. [*mount.remount-rejects-strata]
...
from the mount table will do. [*mount.remount-rejects-identical-strata]
...
at mount and never filtered by what currently exists. [*mount.options-reported-verbatim]

The filesystem type is reported as `stratafs`. [*mount.fstype-name]
...
value is byte-identical to what was supplied. [*mount.options-byte-identical-for-plain-paths] For a path containing any
