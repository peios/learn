...
caller, or by operation. [*strata.stack-fixed-and-ordered]
...
longer than that is refused at parse time. [*strata.max-sixteen] Absence never renumbers
...
and walked from scratch. [*strata.no-cached-resolution]
...
life of the mount and released only when the superblock is freed. [*mount.resolution-context-pinned]
...
deriving inode numbers. [*strata.path-symlinks-followed]
...
depth. [*strata.mount-time-checks] There is no test for a usable directory version value, because
...
the distinction is not observable to a caller. [*strata.stack-errors-are-einval]
...
the provider before it consults the create stratum at all (§4.5.1). [*strata.no-create-stratum-erofs]
...
may sit above the create stratum, below it, or both. [*strata.unflagged-modified-in-place]
...
`ENOENT`. With `am`, an absent directory is accepted at mount time. [*strata.am-required-for-absent-at-mount]
...
means once the mount is live. [*strata.am-not-consulted-at-runtime]
