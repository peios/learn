...
single-emission path; passing one faults. [*kernel-emit.trusted-unvalidated]

## Structural checks and drops [*kernel-emit.structural-checks]
...
incremented. [*kernel-emit.drop-consumes-sequence] That counter also aggregates events discarded by
overwrite when the buffer wraps; it is not exposed in the ring buffer
metadata and is readable only by the KUnit test harness. [*kernel-emit.drop-counter-internal]
...
3. Each event, in order: structural checks, sequence assignment,
   ...
   header size beyond `u32`. [*kernel-emit.batch-stronger-checks] A failing event is dropped exactly as in
   ...
   indicates a kernel bug rather than hostile input. [*kernel-emit.batch-continues-past-failure] This differs
...
the single `write_pos` release store. [*kernel-emit.batch-atomic-visibility] During the batch, the overwrite
...
## Write atomicity [*event.write-atomicity]
