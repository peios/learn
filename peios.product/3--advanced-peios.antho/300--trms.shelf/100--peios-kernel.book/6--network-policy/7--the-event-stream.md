...
attribute, and the status tells that story instead. The layouts are in
§6.A; this page is what they mean.
...
(`BACKSTOP`, `FAIL_CLOSED`, `REJECT_DEGRADED`), direction, address
family, protocol, flow state, interface index, ports, ethertype, both
addresses, the stack-view length, the effect counts the evaluation
yielded packed eight bits each (tags, counts, reports, prompts,
saturating), and the attributing rule's path — the winning rule, or
`backstop`, or `fail-closed` — truncated to 96 bytes.
...
versioned, currently 2), the generation, whether any layer is enforcing,
...
| Group | Counters |
|---|---|
| Seats | `seen_ingress`, `seen_egress`, `seen_local_in`, `deferred` (ingress traversals left for the IP seat), `fallback_judged` (ingress traversals the Packet layer judged there) |
...
Packet-layer evaluations against a live forest, and `permissive` counts
layer evaluations that found no forest — at generation 0, every one.
...
cells exist, so a short buffer is visible.
