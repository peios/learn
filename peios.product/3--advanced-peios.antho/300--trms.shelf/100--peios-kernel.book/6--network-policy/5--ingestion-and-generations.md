...
2. enumerates its children for the layer keys `Packet` and `RawPacket`
   (other names are ignored — someone else's future);
...
and action expression, resolves priority inheritance, lints
layer-impossible facts (a `FlowState` or tag condition in a `RawPacket`
forest — legal, never true; the kernel drops lints, the authoring
surface shows them), and collects the forest's **name sets**: every tag
name mentioned in a `TAG` action, a `PROMPT` fallback, or a `Tag.<n>`
condition; every stream a `COUNT` writes; every distinct counter view a
`Counter.<n>(...)` condition reads, with the first rule and value that
mentioned it. Views are deduplicated and conditions refer to them by
index — that index is what the bridge uses when it fills
`counter_views` (§6.3).
...
After both layers build, `pnp_rust_forests_check()` runs the checks that
span forests, because the stores are machine-wide: tag and stream
hashes must be distinct across *both* forests, and every counter view
must have a writer in *some* forest — a view over a stream no rule
writes is statically dead (it can only ever read absent) and is refused
with the rule and key that read it.

## Publication

`peios_pnp_policy_publish(packet, raw, reporting_level)` is process
context under a mutex. It runs the cross-forest check, then
materializes the counter store for the union of both forests' views
...
new `struct peios_pnp_policy` (two opaque forest pointers plus the
