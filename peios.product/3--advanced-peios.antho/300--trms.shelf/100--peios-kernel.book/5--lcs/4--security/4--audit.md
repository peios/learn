...
| Event | Emitted when |
|---|---|
| `LCS_KEY_OPEN_AUDIT` | A key open matched a SACL audit ACE. [*lcs-audit.key-open.on-sacl-match] |
| `LCS_BACKUP_START` | Before `REG_IOC_BACKUP` reads any subtree data. [*lcs-audit.backup-start.before-any-read] |
| `LCS_BACKUP_COMPLETE` | After a backup completes or fails after starting. [*lcs-audit.backup-complete.after-finish-or-failure] |
| `LCS_RESTORE_START` | Before `REG_IOC_RESTORE` modifies any source state. [*lcs-audit.restore-start.before-any-mutation] |
| `LCS_RESTORE_COMPLETE` | After a restore completes or fails after starting. [*lcs-audit.restore-complete.after-finish-or-failure] |
| `LCS_SOURCE_VALIDATION_FAILURE` | LCS rejected malformed source data. [*lcs-audit.validation-failure.on-malformed-source-data] |
| `LCS_SELF_CONFIG_INVALID` | LCS rejected an invalid self-configuration value. [*lcs-audit.self-config-invalid.on-invalid-value] |
...
GUIDs are
16-byte binary values; SIDs are binary KACS encodings. [*lcs-audit.payload-guid-and-sid-encoding]
...
`impersonation_level` and `integrity_level`. [*lcs-audit.caller-summary-nine-fields]
...
one event arbitrarily large.

A primary token reports an impersonation
level of 0. [*lcs-audit.primary-token-impersonation-level-zero]
...
1 for a failure-audit match, no other bits. [*lcs-audit.key-open.payload-fields]
...
operation code and the key GUID. [*lcs-audit.validation-failure.payload-fields] The last field, `validation_class`,
...
`malformed_value_payload`, `malformed_delete_layer_orphan_list`. [*lcs-audit.validation-failure.twelve-classes]
...
and the value LCS retained instead. [*lcs-audit.self-config.payload-fields]
...
- **`LCS_BACKUP_START`, `LCS_RESTORE_START`.** Emission failure returns
  ...
  is written. [*lcs-audit.emit-failure.start-events-block-the-operation]
- **`LCS_BACKUP_COMPLETE`, `LCS_RESTORE_COMPLETE`.** The operation has
  ...
  result. [*lcs-audit.emit-failure.complete-events-are-best-effort]
