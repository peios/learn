...
1. The slot is marked Down and its hives become unavailable. [*source.failure.slot-marked-down-hives-unavailable]
2. Every pending request fails `EIO`, including ones queued but not yet
   delivered. [*source.failure.pending-requests-fail-eio]
3. **Open key fds stay valid.** An fd holds a GUID and a granted mask,
   ...
   return `EIO` until the source comes back. [*source.failure.key-fds-stay-valid-round-trips-eio]
4. Bound transactions enter `REG_TXN_SOURCE_DOWN`, their poll waiters
   are woken with `POLLERR | POLLHUP`, their mutation logs are
   released, and further use of those fds returns `EIO`. [*source.failure.bound-transactions-enter-source-down]
5. **Watches stay armed.** Watch state is kernel-side and does not
   depend on the source at all. [*source.failure.watches-stay-armed] Nothing is delivered during the window,
...
A late response is validated exactly like an on-time one. [*source.late.validated-like-an-on-time-response] The rules
...
### A late error [*source.late.error-has-no-effects]

The record is released. No watch events, no generation change, nothing.

### A late successful read [*source.late.read-is-validated-then-discarded]
...
calls. [*source.late.replayable-effects-only-for-set-value-and-write-key]

For the other mutating operations — creating, hiding or deleting a path
entry, creating or dropping a key, deleting a value entry, setting a
blanket tombstone, deleting a layer — no effect was retained, so a late
success is a mutation the kernel cannot account for. LCS tears the
source down and returns `EIO` rather than silently ignoring it. [*source.late.unreplayable-mutation-tears-source-down]
...
`RSI_ABORT_TRANSACTION` for that id. [*source.late.begin-transaction-enqueues-an-abort]

A late `RSI_ABORT_TRANSACTION` or `RSI_FLUSH` releases the record with
no effects. [*source.late.abort-or-flush-releases-the-record]
...
events an on-time commit would have produced. [*source.late.commit-applies-retained-commit-effects] Watchers may therefore
...
than ignoring the response. [*source.late.unprocessable-effects-tear-source-down]

A malformed late commit response takes the source down for the same
reason. [*source.late.malformed-commit-tears-source-down]
...
registry-specific leak protection. [*source.failure.no-registry-specific-fd-accounting]

Process exit closes them through normal kernel cleanup: key fds
released and their watches removed, transactions aborted. [*source.failure.process-exit-releases-fds]
