...
into two categories with very different consequences. [*source.validate.every-response-is-validated]
...
- **The source stays alive.** Corruption may be localised, and one bad
  key is not a reason to take a hive offline. [*source.validate.malformed-data-keeps-the-source-alive]

What is checked, by category:

- **Security Descriptors**, from lookups and from layer metadata
  refreshes, must parse and must satisfy the ACE mask rules of §5.4.2. [*source.validate.descriptors-must-parse]
...
- **Names** — layer names, key and child names, value names — must be
  valid under the ordinary rules for their kind. [*source.validate.names-must-be-valid]
- **Sequence numbers** must be below the next number LCS would
  ...
  would decide a winner (§5.3.6). [*source.validate.sequence-number-checks]
- **Payload shape** — an otherwise-matched response whose
  operation-specific payload is the wrong shape, carries trailing
  bytes, or encodes a path target invalidly. [*source.validate.payload-shape-checks]
- **Metadata closure** — a lookup or enumeration whose per-GUID
  metadata block has missing, duplicate, unreferenced or nil entries. [*source.validate.metadata-closure-checks]
  A HIDDEN entry must carry an all-zero GUID and contributes no
  metadata. [*source.validate.hidden-entry-carries-a-zero-guid]
- **Value payloads** — invalid types, a tombstone carrying data, data
  above `MaxValueSize`. [*source.validate.value-payload-checks]
- **Orphan lists** — a nil or duplicated GUID in an `RSI_DELETE_LAYER`
  response. [*source.validate.orphan-list-checks]
- **Status codes** outside the defined vocabulary. [*source.validate.unknown-status-code]
...
transactions enter `SOURCE_DOWN`. [*source.validate.malformed-protocol-is-treated-as-a-crash]
...
source skips them using `total_len`. [*source.validate.requests-may-carry-trailing-fields] That is how a new optional field
