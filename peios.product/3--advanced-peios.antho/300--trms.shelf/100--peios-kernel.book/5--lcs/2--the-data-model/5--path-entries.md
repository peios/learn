...
source stores a **path entry** per layer, which separates the two. [*path-entry.existence-and-naming-are-layer-qualified] The
...
| Child name | The key's name under that parent — one component, not a path. [*path-entry.field.child-name-is-one-component] |
| Layer | The layer this entry belongs to. |
| Target | The GUID of the key at this path in this layer, or HIDDEN. [*path-entry.field.target-is-a-guid-or-hidden] |
...
(§5.A). [*path-entry.hidden.wire-form-is-type-1-and-zero-guid]

A source returning a non-zero GUID on a HIDDEN entry is returning
malformed data. [*path-entry.hidden.non-zero-guid-is-malformed]
...
sequence number, and a key record carrying that GUID. [*path-entry.create.fresh-guid-and-two-records]

LCS sends `RSI_CREATE_ENTRY` first and `RSI_CREATE_KEY`
second. [*path-entry.create.entry-before-key]

That order matters for the race: if another caller got there first, the
entry creation returns `RSI_ALREADY_EXISTS` and LCS retries the whole
thing as an open, reporting `REG_OPENED_EXISTING`. [*path-entry.create.entry-already-exists-retries-as-open] Failure on the
...
resolution decides which is visible. [*path-entry.create.other-layer-gets-a-distinct-guid]
...
a graph. [*path-entry.create.never-points-at-an-existing-guid]
...
regardless of what lower-precedence layers say. [*path-entry.hiding.hidden-entry-makes-a-key-invisible]

It is the path-level equivalent of a value tombstone, and when the
hiding layer is removed the lower key reappears. [*path-entry.hiding.removing-the-layer-restores-the-lower-key]
...
sequence number wins. [*path-entry.hiding.hidden-entry-carries-a-sequence-number]
...
wins on precedence. [*path-entry.hide-and-replace.layer-entry-masks-lower-entries]

Removing the layer removes both the new key and its masking effect, and
the lower key comes back. [*path-entry.hide-and-replace.removing-the-layer-restores-the-lower-key]
...
`(parent, name, layer)` tuple to remove or mask. [*path-entry.hive-root.has-no-parent-or-child-name]

Deleting or hiding a hive root fd is `EINVAL`, rejected before source
dispatch, before transaction enlistment, before sequence allocation and
before any watch event is generated. [*path-entry.hive-root.delete-or-hide-is-einval]
