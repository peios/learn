...
| Name | No | No | The key's own name component. Informational; the authoritative name is in the path entry. [*key.field.name-is-informational] |
| Parent GUID | No | No | The parent key. Nil for a hive root. [*key.field.parent-guid-nil-for-hive-root] |
| Security Descriptor | Yes | No | Computed from the parent at creation; changed at runtime with `WRITE_DAC` / `WRITE_OWNER`. [*key.field.security-descriptor-computed-from-parent] |
| Last write time | Yes | No | Updated when a value is written or deleted or the descriptor changes. [*key.field.last-write-time-updates] |
| Volatile | No | No | The source stores this key in non-persistent storage only. |
| Symlink | No | No | This key is a symbolic link (§5.2.4). |

**No key property is layer-qualified.** Properties belong to the object. [*key.no-property-is-layer-qualified]
...
path at different times are different objects with different GUIDs. [*key.identity.is-guid-not-path] A
...
source at creation, where they become the primary key of its
storage. [*key.identity.guid-assigned-by-lcs-not-source]
...
There is **no persistent retired-GUID catalogue**, and no code anywhere
that would maintain one. A GUID dropped by orphan cleanup is not
remembered. [*key.identity.no-retired-guid-catalogue]
...
inconsistency and fails closed with `EIO`. [*key.identity.create-key-already-exists-is-eio]

It is never retried as an open and `EEXIST` never reaches userspace
from `reg_create_key`. [*key.identity.no-eexist-from-reg-create-key]
...
kernel enforces it. [*key.volatile.storage-is-the-source-obligation]

What the kernel does enforce is the containment rule: **a non-volatile
key may not be created under a volatile parent** (`EINVAL`). [*key.volatile.non-volatile-under-volatile-parent-is-einval]

The converse is allowed — a volatile key under a persistent parent is
ordinary. [*key.volatile.volatile-under-persistent-parent-allowed]

## Naming

Three bytes are forbidden in a key name component: backslash and
forward slash, both of which are separators, and the null
byte. [*key.name.three-forbidden-bytes]

Every other valid UTF-8 sequence is permitted, spaces and arbitrary
Unicode included. [*key.name.any-other-utf8-permitted]

Empty components are forbidden, which rules out a leading separator and
consecutive separators (`Machine\\System`). [*key.name.empty-components-forbidden]

A trailing separator (`Machine\System\`) is likewise
invalid. [*key.name.trailing-separator-invalid]
