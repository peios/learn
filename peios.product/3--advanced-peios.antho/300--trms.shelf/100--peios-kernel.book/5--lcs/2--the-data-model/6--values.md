...
| Name | The value's name; the empty string is the default value. Case-preserving, compared case-insensitively. [*value.field.empty-name-is-the-default-value] |
| Type | A registry value type. |
| Data | An opaque byte array, up to `MaxValueSize` (default 1 MB). [*value.field.data-bounded-by-max-value-size] |
| Layer | The layer this entry belongs to. Every write is tagged. [*value.field.every-write-is-layer-tagged] |
...
one. [*value.at-most-one-unnamed-value-per-key]

## One name, many entries

A single `(key GUID, value name)` pair can have several entries in the
source — one per layer that has written to it. [*value.one-name-many-entries.one-entry-per-writing-layer]

The source stores them all and returns them all. [*value.one-name-many-entries.source-returns-every-entry]

LCS resolves the effective value at read time (§5.3.6).
...
`REG_RESOURCE_REQUIREMENTS_LIST` and `REG_QWORD`, numbered 0 to 11. [*value.types.full-windows-set-numbered-0-to-11]
Values are in §5.A.

LCS stores the type tag and returns it on read but does not interpret
the data. [*value.types.tag-stored-and-returned-uninterpreted] The single exception is `REG_LINK` read as a symlink key's
...
produces one, and for every operation they behave as `REG_BINARY`. [*value.types.hardware-resource-types-behave-as-reg-binary]
...
is `EINVAL`. [*value.types.unknown-type-code-is-einval]
...
is a tombstone gets `ENOENT`, which is exactly what a tombstone
means. [*value.reg-tombstone.never-returned-to-a-caller]
...
operation. [*value.reg-tombstone.written-by-setting-the-type]

It must carry zero-length data; non-empty tombstone data is `EINVAL`,
again before sequence allocation, enlistment or dispatch. [*value.reg-tombstone.non-empty-data-is-einval]

## Naming

Value names use the same case rules as key names: Unicode Simple Case
Folding, case-preserving, case-insensitive. [*value.name.same-case-rules-as-key-names]
...
no meaning in one. [*value.name.separators-are-permitted]

Only the null byte is forbidden, and invalid UTF-8 is rejected. The
empty string is reserved for the default value. [*value.name.only-the-null-byte-is-forbidden]
