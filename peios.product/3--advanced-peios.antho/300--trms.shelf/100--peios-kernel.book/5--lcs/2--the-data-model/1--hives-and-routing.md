...
no static configuration. [*hive.routing-table.built-from-registrations]

| Field | Description |
|---|---|
| Name | The hive name, e.g. `Machine`, `Users`. Case-preserving, compared case-insensitively. [*hive.field.name-case-insensitive] |
| Root GUID | The GUID of the hive's root key, supplied by the source at registration. [*hive.field.root-guid-supplied-by-source] |
| Source | The source slot backing this hive. |
| Status | Active or Unavailable, tracking source connectivity. [*hive.field.status-active-or-unavailable] |
...
source's, and they go Down together. [*hive.status-is-per-source-slot]
...
GUID. [*hive.route-identity.name-and-scope]

That pair must be unique across every registered source; a source
claiming one another source already holds is rejected. [*hive.route-identity.must-be-unique]

The same name may appear in different scopes, which is precisely how a
private hive shadows a global one without colliding with it. [*hive.route-identity.same-name-in-different-scopes]

A hive is backed by exactly one source. A source may back many. [*hive.one-source-per-hive]

## Routing a path

LCS takes the first component of a path — everything before the first
separator — and looks it up. [*hive.routing.uses-first-component]

- Registered and active routes to the backing source. [*hive.routing.active-routes-to-source]
- Not registered is `ENOENT`. [*hive.routing.unregistered-is-enoent]
- Registered but Down is `EIO`. [*hive.routing.down-is-eio]
...
routing. [*hive.currentuser.rewrites-to-users-sid]

The rewritten path is re-checked against the total-length limit, since
a textual SID is longer than the alias it replaced. [*hive.currentuser.rewritten-path-rechecked-for-length]

Three constraints keep this safe.

It applies **only to the first component** of a path, and only to a
caller-supplied absolute one. [*hive.currentuser.first-component-of-caller-path-only]

It does **not** apply to symlink targets: a target beginning with
`CurrentUser\` is followed literally, routes as a hive of that name,
and finds nothing. [*hive.currentuser.not-applied-to-symlink-targets] Without that rule, a symlink
...
hive. [*hive.currentuser.cannot-be-registered-einval]

Symlink targets *are* subject to ordinary hive routing, private hives
included. [*hive.symlink-targets-use-ordinary-routing] A sandboxed process resolving a symlink sees the same
...
nowhere like any other name — but the two names do exist in the kernel. [*hive.known-names.get-no-routing-privilege]
...
than `MaxPathComponentLength`. [*hive.name.follows-key-component-rules] They are case-preserving and compared
