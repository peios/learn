...
Sixteen ioctls act on a key fd. [*ioctl.sixteen-act-on-a-key-fd] Each checks the fd's granted mask
...
Every mutating ioctl accepts an optional transaction fd. [*ioctl.mutating-ioctls-accept-a-transaction-fd] So do the four
...
| `ENOMEM` | Kernel allocation failure. [*ioctl.enomem-on-allocation-failure] |
...
| `ENOTTY` | An ioctl number this fd type does not implement. [*ioctl.enotty-for-an-unimplemented-number] |
...
spelling, not whatever string the source stored. [*ioctl.query-value.result-fields] A base-layer entry
therefore reports `base`.

A winning tombstone or blanket, or no
entries at all, is `ENOENT`. [*ioctl.query-value.masked-is-enoent]
...
blanket-masked values omitted. [*ioctl.query-values-batch.returns-every-effective-value] This is the call to use when you want
them all (§5.3.6).

**`REG_IOC_ENUM_VALUES`** and **`REG_IOC_ENUM_SUBKEYS`** return the
entry at an index, and `ENOENT` past the end. [*ioctl.enum.index-and-enoent-past-the-end]

Both re-resolve the full
set on every call, and enumeration order is undefined — see §5.3.6 for
why the batch call usually wins. [*ioctl.enum.re-resolves-and-order-is-undefined]

`ENUM_SUBKEYS` performs no per-child access check. It returns every
visible child with its last write time, subkey count and value count. [*ioctl.enum-subkeys.child-fields]
...
volatile and symlink flags, and the hive generation number. [*ioctl.query-key-info.result-fields] It requires
`READ_CONTROL`.
...
back. [*ioctl.query-key-info.is-iowr]

It was declared `_IOR`, and because the direction bits are part of
...
carrying the new one. [*ioctl.query-key-info.old-constant-gets-enotty]
...
contained. [*ioctl.hive-generation.one-increment-per-commit]

Its baseline is initialised from the source's reported
maximum sequence at registration so that observed values are monotonic
relative to persisted entries; saturating at `U64_MAX` is `EOVERFLOW`. [*ioctl.hive-generation.saturation-is-eoverflow]
...
key is gone but the layer's entries are still resolving. [*ioctl.hive-generation.layer-operation-is-one-increment]

An operation
affecting several hives increments each independently. [*ioctl.hive-generation.per-hive-independent]
...
the key's last write time. [*ioctl.set-value.stores-the-layer-tuple]
...
- `EINVAL` for an unknown type or a tombstone with data.
- `EAGAIN` for a failed conditional write.
- `ENOSPC` for the layer cap or oversized data. [*ioctl.set-value.enospc-layer-cap-or-oversized-data]
- `ENAMETOOLONG` for the value name. [*ioctl.set-value.enametoolong-value-name]
- `EPERM` for a `Precedence` above 0 without `SeTcbPrivilege` (§5.3.4).

**`REG_IOC_DELETE_VALUE`** removes one layer's entry at a value name —
whether that entry was a value or a tombstone. [*ioctl.delete-value.removes-one-layers-entry] Removing a layer's
...
kernel behaviour. [*ioctl.delete-value.source-not-found-is-enoent]
...
**`REG_IOC_DELETE_KEY`** removes this key's path entry from a layer. [*ioctl.delete-key.removes-the-path-entry]

The parent GUID comes from the fd's ancestor chain and the child name
from the last component of its resolved path. [*ioctl.delete-key.parent-and-name-from-the-fd] `ENOTEMPTY` if the key
has visible children; `EINVAL` on a hive root (§5.2.9).

**`REG_IOC_HIDE_KEY`** creates a HIDDEN path entry at the same place
instead, masking lower-precedence entries. [*ioctl.hide-key.creates-a-hidden-entry] The caller must hold an open
fd to the key, which is how it proved access to it. [*ioctl.hide-key.requires-an-open-fd] `EINVAL` on a hive
root.
...
key's hive, and returns when persistence is confirmed. [*ioctl.flush.persists-and-waits-for-confirmation] The hive name
comes from the first component of the fd's resolved path. [*ioctl.flush.hive-from-the-fd-path] It requires
`KEY_SET_VALUE` (§5.4.2).
