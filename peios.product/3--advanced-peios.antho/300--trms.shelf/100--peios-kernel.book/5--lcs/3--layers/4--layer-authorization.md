...
metadata key at `Machine\System\Registry\Layers\<LayerName>\`. [*layer.authz.write-requires-key-set-value-on-metadata-key]
...
|---|---|
| Create a layer at precedence 0 | `KEY_CREATE_SUB_KEY` on `Layers\` [*layer.authz.create-requires-create-sub-key] |
...
| Modify layer metadata | `KEY_SET_VALUE` on the metadata key; raising precedence above 0 additionally requires `SeTcbPrivilege` [*layer.authz.modify-metadata-requires-key-set-value] |
| Delete a layer | `DELETE` on the metadata key's fd [*layer.authz.delete-requires-delete-on-the-metadata-fd] |

Everything except the precedence rule is controlled purely by the
descriptor on the metadata key. [*layer.authz.only-precedence-needs-a-privilege]

## The precedence gate

`SeTcbPrivilege` is required specifically to establish or raise a
layer's precedence above 0. [*layer.authz.precedence-above-zero-requires-setcbprivilege] It is defence in depth: compromising the
...
metadata. The precedence never actually rises. [*layer.authz.non-dword-precedence-slips-the-gate-and-fails-at-refresh]
...
last path entry. [*layer.authz.deleting-the-metadata-key-removes-the-layer-from-the-table]
