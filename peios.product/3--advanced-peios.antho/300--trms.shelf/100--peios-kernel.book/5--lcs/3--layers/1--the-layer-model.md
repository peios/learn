...
|---|---|---|
| Name | No | The layer's identity — not a GUID, not an integer. Case-preserving, compared with Unicode Simple Case Folding. Bounded by `MaxPathComponentLength`. [*layer.model.name-is-the-identity] |
...
descriptor on the layer's metadata key (§5.3.4). [*layer.model.owner-never-used-for-an-access-check]

It is also not quite immutable as a field: a refresh re-reads the
`Owner` value and re-selects it, so rewriting the value does change what
is cached. [*layer.model.owner-re-selected-by-refresh] Because nothing
consults it, that has no effect on anything.
...
contacted, by querying the source for the current entry count. [*layer.model.max-layers-per-value-checked-at-set-value]

A write that replaces an existing entry in the *same* layer does not
increase the count and is not checked. [*layer.model.max-layers-per-value-same-layer-replace-exempt]
Exceeding the cap is `ENOSPC`. [*layer.model.max-layers-per-value-exceeded-is-enospc]

The check is deliberately **best-effort admission control**, not a
storage invariant. [*layer.model.max-layers-per-value-is-best-effort] It queries and then dispatches without holding
anything, so concurrent writers can both observe room and both proceed.
Sources are not required to enforce it atomically.

Once LCS observes a count at or above the cap, further new-layer writes
are refused. [*layer.model.max-layers-per-value-refuses-once-observed-full]

Blanket tombstones and value deletions are not subject to it. [*layer.model.max-layers-per-value-exempts-blankets-and-deletions]

## Layers are global; entries are per-source [*layer.model.table-is-global-entries-are-per-source]
...
RSI operation that hands a source the layer list. [*layer.model.no-rsi-operation-sends-the-layer-list]
