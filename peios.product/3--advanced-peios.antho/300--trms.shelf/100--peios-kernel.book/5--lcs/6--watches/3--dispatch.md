...
at the moment of the event. [*watch.dispatch.computed-from-identity-and-captured-ancestry]

## A watch is bound to an object

A watch is registered against the GUID on the fd it was armed through.
It fires for changes to *that object*. [*watch.dispatch.registered-against-the-fd-guid]

If a layer change causes a different key to become visible at the same
path, the watch does not move: the new key is a different object with a
different GUID, and nothing about the old watch refers to a name. [*watch.dispatch.watch-does-not-follow-the-path]
...
delivered, but the watch is not removed. [*watch.dispatch.hiding-keeps-the-watch-armed]

Should the hiding layer later be deleted, the key reappears at its
original path with the same GUID, and events resume. [*watch.dispatch.events-resume-when-the-key-reappears]
There is no re-emergence event; the watcher simply observes the key
being active again. [*watch.dispatch.no-re-emergence-event]
...
root GUID through parent GUID to the key itself. [*watch.dispatch.ancestor-chain-captured-at-open]
It costs nothing extra: the walk had to resolve those components
anyway.

A relative open copies the parent fd's chain and extends it with the
components the relative walk resolved. [*watch.dispatch.relative-open-extends-the-parent-chain]

An open that followed a symlink records the chain of the *resolved*
path, so the fd's ancestry reflects the target's real position in the
tree, not the link's. [*watch.dispatch.symlink-open-records-the-resolved-chain]
...
opened through that chain. [*watch.dispatch.stale-ancestor-still-receives-subtree-events]

Watches on a new key that later appears at the same path receive
nothing from the old one. [*watch.dispatch.new-key-at-the-same-path-receives-nothing]
...
1. Look up *B*'s GUID in the watch map and queue the event, with
   `path_depth` 0, to every watcher whose filter admits it. [*watch.dispatch.direct-watchers-receive-depth-zero]
2. Walk the ancestor chain outward from *B*'s parent. At each ancestor,
   ...
   filter admits it, with the path from that ancestor down to *B*. [*watch.dispatch.subtree-watchers-receive-the-relative-path]

The cost is O(depth) hash lookups, with no RSI round trips, no trie and
no string comparison. [*watch.dispatch.costs-o-depth-hash-lookups-and-no-source-io]
The path components handed to a subtree watcher are sliced out of the
...
watch is told about. The default is 0, meaning unlimited. [*watch.dispatch.depth-zero-means-unlimited]

A non-zero value suppresses events whose path is longer than it,
limiting both noise and dispatch cost for a watch armed high in a
tree. [*watch.dispatch.depth-limit-suppresses-deeper-events]
...
Nothing inside an uncommitted transaction dispatches. [*watch.dispatch.uncommitted-transaction-dispatches-nothing]

At commit, and only when the source reports success, the transaction's
mutation log is walked in operation order and the whole set of events
...
`OVERFLOW` and none of its individual events. [*watch.dispatch.burst-cap-replaces-a-watchers-batch-with-one-overflow]

That `OVERFLOW` is queued ahead of the batch, so it arrives before
whatever else that watcher is still owed. [*watch.dispatch.burst-overflow-queued-ahead-of-the-batch]
...
a subtree from a stream are all of this kind. [*watch.dispatch.recovery-triggering-operations]
...
It does not disarm anything. [*watch.dispatch.recovery-does-not-disarm]
...
operations needing the source return `EIO`. [*watch.dispatch.disconnect-keeps-watches-armed-and-silent]

`OVERFLOW` arrives on **re-registration**, not on disconnect, which is
the only ordering that works: a watcher told to re-read needs the
source to be there when it does. [*watch.dispatch.overflow-arrives-on-re-registration]

Existing fds resume without re-opening, and no watcher has to
re-arm. [*watch.dispatch.fds-resume-without-re-arming]
