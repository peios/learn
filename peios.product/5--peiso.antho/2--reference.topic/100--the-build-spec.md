...
**`peiso.toml`** is the declarative TOML spec that drives [`peiso`](~peios/building-images/overview). A single file describes the whole bootable image: the [peipkg-compose manifest](~peios/package-management/composing-a-root) to build the package root from, how the initramfs is packed, and the optional squashfs, UKI, ISO, registry-seed and feature stages that [the build pipeline](~peios/building-images/the-build-pipeline) layers on top. This page documents **schema version `1`** — the only schema peiso accepts — field by field.
...
> [!IMPORTANT]
> A booted system can only read the data area if something mounts the medium. On a live image that is [live-boot](~peios/boot-and-trust-establishment/initramfs-stage)'s root-mount hook, which carries the medium to `/media/peios` after the pivot — prelude moves only `/proc`, `/sys` and `/dev` into the new root and then chroots, so a mount nobody relocates is unreachable afterwards.
...
A spec that passes all of these checks is what [the build pipeline](~peios/building-images/the-build-pipeline) runs.

## See also

- [Building a Peios image](~peios/building-images/overview) — what peiso is and where it sits above compose.
- [The build pipeline](~peios/building-images/the-build-pipeline) — the stage each table drives, in run order.
- [Running a build](~peios/building-images/running-a-build) — the command, exit codes, and required tools.
