...
type: concept
description: Each privilege on a token is absent, present-disabled, present-enabled, or used — the transitions between states, AdjustPrivileges, and why removal is one-way.
related:
...
FilterToken is covered in [Token lifecycle](~peios/security-fundamentals/tokens/lifecycle) and [Restricted and write-restricted tokens](~peios/tokens/restricted-tokens).
