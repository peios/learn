...
title: Opening files
type: reference
description: kacs_open with an explicit desired mask versus legacy openat with POSIX flags — strict-mode grants, create dispositions, MAXIMUM_ALLOWED, and SD-at-creation.
related:
...
- [The handle model](~peios/file-access/the-handle-model) — what the cached mask these opens produce actually does.
- [Managing file security](~peios/file-access/managing-file-security) — reading and writing the SD behind a handle.
- [DACL evaluation](~peios/security-descriptors/dacl-evaluation) — the maximum-allowed walk MAXIMUM_ALLOWED relies on.
