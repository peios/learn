...
Note that a module being *available* in the initramfs is not the same as it being loaded. Everything the boot path currently needs is built into the kernel image itself, so a Peios kernel reaches its root with no modules loaded at all.
