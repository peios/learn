...
type: concept
description: The axes a token is classified along — primary or impersonation, impersonation level, restricted, elevation type — and the token's fields grouped by purpose.
related:
  - peios/tokens/overview
  - peios/security-fundamentals/tokens/lifecycle
  - peios/tokens/restricted-tokens
  - peios/security-fundamentals/tokens/elevation
...
For how these fields are set, shared, adjusted, and destroyed over a token's life — mint, fork, exec, impersonation, adjustment — read [Token lifecycle](~peios/security-fundamentals/tokens/lifecycle).
