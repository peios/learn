...
type: concept
description: A logon session is the kernel object recording one authentication event — every token belongs to exactly one, tied together by the session's logon SID.
related:
