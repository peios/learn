...
For the wire-level change stream — cursors, event shapes, heartbeats — read the [HTTP API reference](~wispist/reference/http-api#change-stream).

For the stream caps and retention bounds behind `reset`, read [Limits and abuse controls](~wispist/reference/limits).
