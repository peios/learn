...
For the client methods that carry these semantics — `get`, `create`, `replace`, `update`, `delete` — read the [Browser API reference](~wispist/reference/browser-api).

For what happens when a conditional mutation loses a race, read [Revision conflict](~wispist/problems/revision-conflict).

For how draft and live namespaces relate across releases, read [Drafts and publishing](~wispist/using-wispist/drafts-and-publishing).
