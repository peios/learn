...
type: concept
description: "The [files] and [symlinks] tables, source-ref syntax, globs, excludes, and the destination and collision rules that turn staged output into a package payload."
related:
...
[Package management](~peios/package-management/overview).

## Where to go next

For the rest of the manifest — dependencies, provides, claims — read
[Dependencies and claims](~pekit/recipes/dependencies-and-claims).

For splitting one recipe into several packages, read
[Multi-package recipes](~pekit/recipes/multi-package).

For the exhaustive key-by-key schema, read the
