...
type: concept
description: "The files that make up a recipe, pekit.toml section by section, and the PEKIT_* environment contract every target command runs against."
related:
...
| `workspace.pekit.toml` | Sits **one level up** and groups a directory of recipes into a workspace pekit can drive as a unit. | [Workspaces](~pekit/running/workspaces) |
...
- **`[source_package]`** — opt out of, or rename, the corresponding-source package a recipe with a reproducible source emits alongside its binary packages. See [Signing and provenance](~pekit/running/signing-and-provenance).
...
The full flag and locator surface is on the [invocation](~pekit/running/invocation) page.
