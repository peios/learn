...
type: concept
description: "How a package declares what it needs, what pekit derives from the built binaries, and how packages take part in shared filesystem names through claims."
related:
...
- **pkgconfig derivation** — emits `pkgconfig(<name>)` capabilities from any
  `.pc` files in the payload.
...
For the workspace `[policy]` table that refines soname derivation, read [Workspaces](~pekit/running/workspaces).
