...
type: concept
description: "Base and member package files, layer merge and precedence, selecting members with positional selectors and --all, and the [multipack] instance fan-out."
related:
...
Discovery recognises two kinds of file:
...
1. workspace base
2. source base (only when the recipe [delegates](~pekit/running/workspaces)
...
For the `package` and `publish` commands and their selectors, read [Commands and targets](~pekit/running/commands-and-targets).

For driving the packages of many recipes at once, read [Workspaces](~pekit/running/workspaces).
