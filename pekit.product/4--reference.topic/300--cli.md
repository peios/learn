...
type: reference
description: "Every pekit flag with its value form, the command-by-flag capability matrix, validation rules, exit codes, and the full PEKIT_* environment contract."
related:
...
and recipe location, see [Invocation and flags](~pekit/running/invocation);
...
`clean`, `gen`, `verify`, `lock`, `lint`, `workspace`, `help`, or `version`. Flags may appear before or after the command word and
...
| Invocation | Output |
|---|---|
| `pekit help`, `pekit --help`, `pekit -h` | The overview: every command with a line each, the global flags, and a pointer to this page. Exit `0`. |
| `pekit help <command>`, `pekit <command> --help` | That command's usage line, what its selectors mean, and its flags. Generated from the same tables the parser uses, so it cannot list a flag the command rejects. Exit `0`. |
| `pekit` (no arguments) | The overview on **stderr**, exit `1` (`missing_command`): nothing ran. |
| `pekit version` | `pekit <module version> (<commit>[, modified])`, from the binary's build info. `--version` and `-V` are the version *selector*, so the print form is a command. |

`--help` wins over validation: `pekit clean --version 1 --help` prints help
rather than `unsupported_flag`. `help` and `version` accept the global flags
and no others; `workspace` cannot delegate to either.
...
| `--no-build[=<list>]` | Optional-value | Reuse already-staged build targets. Bare form reuses all; `=a,b,c` reuses only the named build targets. Only affects `build`-kind targets whose stage already exists, and a stage left behind by a target that did not finish is refused with `stage_incomplete` rather than reused. |
| `--no-verify[=<list>]` | Optional-value | Skip the [gen drift-check pre-flight](~pekit/running/commands-and-targets#generating-source-gen-and-verify). Bare form skips all; `=a,b,c` skips only the named gen targets (an unknown name errors). On `package`/`publish` it passes through to the builds they trigger. |
| `--no-gates` | No-value | `package`/`publish` only: skip every `gate = true` test in the recipe for rapid iteration. Pekit warns with the skipped gate names. |
| `--env <name>` | Required-value | Select environment-file layers. `main` (default) loads optional `env.pekit.toml`; `<name>` loads `<name>.env.pekit.toml`, which must exist at the workspace or recipe root; a recipe file overlays the workspace file. `none` loads no env files. |
...
| `--refresh-source` | No-value | Force source re-materialisation, ignoring any source cache. The [lockfile](~pekit/reference/supporting-files#pekit-lock) is still enforced against the fresh download. |
...
| `--allow-unsigned` | No-value | Permit `publish` of peipkg-format packages without a configured [signing key](~pekit/reference/supporting-files#well-known-entries) (otherwise `unsigned_publish`). |
...
[Workspaces](~pekit/running/workspaces).
...
| Command | version selection | local source | `--no-build` | `--no-verify` | `--no-gates` | `--env` | `--keyring` | `--refresh-source` | `--allow-unanchored` | `--allow-unsigned` | `--all` | clean mode | `--repin` |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| `build` | Yes | Yes | Yes | Yes | | Yes | Yes | Yes | | | | | |
| `test` | Yes | Yes | Yes | Yes | | Yes | Yes | Yes | | | | | |
| `install` | Yes | Yes | Yes | Yes | | Yes | Yes | Yes | | | | | |
| `package` | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | | | Yes | | |
| `publish` | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | Yes | | |
| `clean` | | | | | | Yes | Yes | | | | | Yes | |
| `gen` | | | | | | Yes | Yes | | | | Yes | | |
| `verify` | | | | | | Yes | Yes | | | | Yes | | |
| `lock` | Yes | | | | | | | Yes | | | | | Yes |
...
"clean mode" is the `--output-only` / `--target-only` pair. Plain `lint`
acquires delegated recipe files automatically. Its version-selection and
local-source flags make that source choice explicit and additionally select an
existing build stage for [payload rules](~pekit/running/linting); it never
builds.
...
| `lint` with any selector | `invalid_selector` (lint checks every package) |
| `lint --all` | `unsupported_flag` |
| `help <name>` where `<name>` is not a command | `unknown_command` |
| `version` with any selector | `invalid_selector` |
| `workspace help` or `workspace version` | `missing_workspace_command` |
...
| Variable | Value |
|---|---|
| `PEKIT_VERSION` | Full version string, including fourth and later numeric components. |
| `PEKIT_VERSION_MAJOR` | First numeric component. |
| `PEKIT_VERSION_MINOR` | Second numeric component. |
| `PEKIT_VERSION_PATCH` | Third numeric component. |
| `PEKIT_VERSION_SUFFIX` | Unseparated upstream suffix (may be empty). |
...
- [Invocation and flags](~pekit/running/invocation) — the narrative walk-through of the grammar.
- [Commands and targets](~pekit/running/commands-and-targets) — what each command selects and runs.
- [Environments and keyrings](~pekit/recipes/environments-and-keyrings) — the layers behind the environment contract.
