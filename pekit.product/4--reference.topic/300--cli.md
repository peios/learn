...
"clean mode" is the `--output-only` / `--target-only` pair. `lint` takes
version selection and local-source flags only to find an existing build stage
for its [payload rules](~pekit/running/linting); it never builds.
