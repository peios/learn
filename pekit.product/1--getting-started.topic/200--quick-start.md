...
- [Commands and targets](~pekit/running/commands-and-targets) — the commands, named targets, `needs` ordering, target selection, and the `gen`/`verify` source-generation pair.
- [Invocation and flags](~pekit/running/invocation) — the command line: `--dry-run`, `--json`, and how recipes are located.
