...
AccessCheck compares both axes numerically. The object is PIP-dominant only if
the caller's `pip_type >= type` and `pip_trust >= trust`.
...
These named values are conventional labels, not a closed enum for AccessCheck.
Other numeric `type` values are valid and MUST be compared numerically using the
same dominance rule.

A trust-label SID with any other shape is invalid. AccessCheck MUST treat the
security descriptor as malformed rather than guessing.
...
If multiple SYSTEM_PROCESS_TRUST_LABEL_ACEs appear in the SACL, only the first
non-inherit-only ACE MUST be used. Inherit-only trust-label ACEs do not apply
