...
Requires SeCreateTokenPrivilege. The kernel validates: all SIDs well-formed, owner/primary_group indices valid, `auth_id` references an existing session that is not dead, Primary tokens have `impersonation_level` = Anonymous, `write_restricted` = true requires `user_deny_only` = true, `isolation_boundary` = true requires `confinement_sid` present, wire format `_reserved1` (elevation_type) must be 0. See the Token Creation section for the full validation list.
...
### kacs_invalidate_session

Marks a logon session as dead. All future AccessCheck evaluations against tokens referencing this session will be denied. Token creation and installation against the session are also rejected.

| Parameter | Description |
|---|---|
| `auth_id` | Session ID (LUID) to invalidate. |
| Returns | 0 on success, `-errno` on failure. |

Requires SeTcbPrivilege. Invalidating an already-dead session is a no-op (returns 0). Invalidating a nonexistent session returns `-ENOENT`. Emits a session-invalidated event via `event_emit`.
