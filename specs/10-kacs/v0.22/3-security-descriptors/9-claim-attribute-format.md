...
|---|---|---|---|
| 0 | 4 | `NameOffset` | Offset to the UTF-16LE null-terminated attribute name. Maximum name length is 256 UTF-16LE code units (512 bytes) including the null terminator. Names exceeding this limit make the claim entry invalid. |
...
| 12 | 4 | `ValueCount` | Number of values. May be 0. Maximum is 1024. Entries with `ValueCount` exceeding 1024 are invalid. This prevents integer overflow in the `4 * ValueCount` offset array size computation. |
