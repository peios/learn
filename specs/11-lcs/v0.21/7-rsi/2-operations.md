...
This is the only RSI operation that identifies its target by hive
name rather than key GUID, because flush is a hive-level operation
(e.g., WAL checkpoint), not a key-level one. LCS derives the hive
name from the key fd used in the REG_IOC_FLUSH ioctl.
