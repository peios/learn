...
| MaxNestingDepth | REG_DWORD | 32 | 4--256 | Maximum permitted msgpack nesting depth for payloads emitted via the `kmes_emit` and `kmes_emit_batch` syscalls, counted as defined in §2.1 Payload. Payloads exceeding this depth are rejected. Does not apply to kernel emitters. |
