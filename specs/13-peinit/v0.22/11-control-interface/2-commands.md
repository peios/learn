...
includes `"mode": "confirmed"` or `"mode": "advisory"` (see the
Restart and Reload section).
...
- **QUEUE:** operation is queued as Pending. Executes after the
  current operation completes (see the Conflict Resolution section
  in Operations).
