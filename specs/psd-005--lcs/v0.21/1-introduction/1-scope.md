...
- loregd internals (PSD-006 — SQLite schema, storage engine, caching,
  connection management)
- Specific registry key schemas (those belong to the subsystems that own
  them)
- KACS (PSD-004)
- peinit boot decisions (PSD-007 — first-boot detection, seed restore
  triggering, service start ordering)
