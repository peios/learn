...
2. **SYSTEM LogonSession.** The SYSTEM LogonSession (LUID 999) is created by direct kernel initialization (not via `kacs_create_logon_session` — no process or privilege exists yet): logon_session_id = 999, logon type SERVICE, user SID `S-1-5-18`, auth package "Negotiate". Logon SID `S-1-5-5-0-999` is derived from logon_session_id.

3. **SYSTEM token.** The SYSTEM token is minted by direct kernel initialization (not via `kacs_create_token`): user `S-1-5-18`, all privileges from the privilege catalog enabled, integrity System, token type Primary, impersonation level Anonymous, auth_id 999 (the SYSTEM LogonSession), source `PeiosKrn` (source LUID = 0), projected UID 0, projected GID 0, elevation_type Default, mandatory_policy `NO_WRITE_UP | NEW_PROCESS_MIN` (0x0003), interactivity_scope 0, expiration 0 (no expiry), origin LUID 0, audit_policy 0, write_restricted false, user_deny_only false, restricted_sids empty, confinement_sid absent, confinement_exempt false, isolation_boundary false, projected_supplementary_gids empty. Its token identity/default fields are: token_id 0, token_guid (kernel-generated UUIDv4), modified_id 0, owner_sid_index 0 (the user SID), primary_group_index 1 (the first group SID, Administrators), and default_dacl equal to the binary ACL listed below. Groups:
   ...
   - `S-1-5-5-0-999` (Logon SID) — SE_GROUP_MANDATORY | SE_GROUP_ENABLED_BY_DEFAULT | SE_GROUP_ENABLED | SE_GROUP_LOGON_ID

   Default DACL (for objects created by SYSTEM): ALLOW SYSTEM GENERIC_ALL, ALLOW BUILTIN\Administrators GENERIC_ALL. Its binary ACL is revision 2, size 52, ace_count 2, with ACCESS_ALLOWED_ACE GENERIC_ALL for SYSTEM followed by ACCESS_ALLOWED_ACE GENERIC_ALL for BUILTIN\Administrators:

   `02 00 34 00 02 00 00 00 00 00 14 00 00 00 00 10 01 01 00 00 00 00 00 05 12 00 00 00 00 00 18 00 00 00 00 10 01 02 00 00 00 00 00 05 20 00 00 00 20 02 00 00`
