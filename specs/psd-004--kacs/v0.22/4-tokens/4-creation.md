...
**Kernel-generated fields:** `token_id` (LUID), `modified_id` (initialized to `token_id`), `created_at` (current time), `elevation_type` (always Default), `logon_sid` (derived from `logon_session_id` as `S-1-5-5-{logon_session_id >> 32}-{logon_session_id & 0xFFFFFFFF}`), token SD (default SD per the Token Access Rights section).
