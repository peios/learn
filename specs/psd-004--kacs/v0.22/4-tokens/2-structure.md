...
Linked token pairs are associated at the LogonSession level, not stored on individual tokens. See §4.6 for the pairing mechanism.
...
| Field | Type | Description |
|---|---|---|
| `token_guid` | UUID | 128-bit identifier for this token instance. UUIDv4 per RFC 4122, generated by the kernel at creation time. Immutable. Used by KMES for identity stamping and event correlation. |
...
| Field | Description |
|---|---|
| `modified_id` | Monotonic u64 counter. Initialized to 0 at creation, incremented on any token adjustment. Serves as a cache invalidation key — if `modified_id` has changed since the last AccessCheck, cached decisions are stale. |
