...
- **Token creation.** `kacs_create_token` rejects a `auth_id` that references a dead LogonSession. No new tokens can enter the system referencing a dead LogonSession.
...
- **Event.** When a LogonSession is invalidated, the kernel emits a LogonSession-invalidated event through KMES.
...
3. authd enumerates processes whose tokens carry the target `auth_id` or `interactivity_scope` by walking `/proc/*/token`, opening each node's query-only inspection handle, and inspecting `TokenStatistics` (which includes `auth_id`). No dedicated enumeration syscall is needed.
...
> [!INFORMATIVE]
> Token file descriptors can be passed between processes via IPC. A token reference held by a process outside the target LogonSession will survive the LogonSession's process termination. The dead-LogonSession check ensures these orphaned tokens cannot be used for new access checks, even though existing open fds retain their cached granted masks.
