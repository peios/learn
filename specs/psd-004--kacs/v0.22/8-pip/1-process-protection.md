...
PIP types form a total order by their numeric values. `None` (0), `Protected` (512), and `Isolated` (1024) are the conventional tiers used by v0.22, but `pip_type` is not a closed enum: nonzero values are compared directly as numeric protection tiers. PIP trust levels are also numeric and compared directly (higher = more trusted). The early return for `pip_type = None` ensures that unprotected processes are always accessible regardless of trust values. The two-dimension check applies whenever the target `pip_type` is nonzero.
