...
| SE_DACL_TRUSTED (DT) | 6 | 0x0040 | Reserved metadata. Preserved during round-trip serialization. No operational semantics in v0.20. |
...
v0.20 records the `SE_SERVER_SECURITY` flag value but does not define the server-ACL construction algorithm. v0.20 implementations MUST fail closed when a creator SD attempts to use `SE_SERVER_SECURITY`.
