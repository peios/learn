...
|---|---|---|
| `process_guid` | UUID | 128-bit identifier for this process instance. UUIDv4 per RFC 4122, generated by the kernel at fork. Immutable for the lifetime of the process. NOT copied from the parent — every process receives a new GUID. Used by KMES and kernel-internal consumers for identity stamping and event correlation. |

The Process GUID is distinct from the PID. PIDs are recycled; Process GUIDs are unique within a boot (globally unique preferred). The GUID is the stable correlation key that KMES and kernel-internal consumers use to attribute events to a process across its lifetime.
