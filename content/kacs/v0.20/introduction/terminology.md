...
**ACE** (Access Control Entry): A single rule within an ACL. Contains: a type (allow, deny, audit, or system), a SID, and an access mask.
...
**FACS** (File Access Control Shim): The KACS subsystem that replaces Linux DAC with SD-based evaluation on files. Enforces the handle model: AccessCheck runs at open time, the granted mask is cached on the file descriptor, and subsequent operations check the cached mask.
