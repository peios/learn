...
If the SD has no DACL (SE_DACL_PRESENT not set), all valid rights that have not already been decided by earlier pipeline stages are granted. The valid rights are bounded to `MapGenericBits(GENERIC_ALL, mapping)`, not raw 0xFFFFFFFF.
...
By default, the owner of an object receives READ_CONTROL and WRITE_DAC regardless of what the DACL says. These implicit rights are granted **before** the DACL walk. Because first-writer-wins governs the walk, a deny ACE encountered during the walk cannot override owner implicit rights.

If any non-inherit-only access-control ACE in the DACL targets the OWNER RIGHTS SID (`S-1-3-4`), the implicit grant is suppressed entirely. The check is a pre-scan of the DACL -- it checks for the ACE's presence, not whether any condition on the ACE evaluates to TRUE.

During the walk, S-1-3-4 is treated as a normal SID matching the owner.
