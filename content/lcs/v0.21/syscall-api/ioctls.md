...
bound to a different source than the key's source, the ioctl
returns EXDEV.
...
| EXDEV | Transaction fd is bound to a different source (mutating ioctls only). |
...
effective sequence (uint64), effective layer name (string).
...
regardless of how many operations it contains. Layer deletion
effects and layer precedence changes that alter effective state
also increment by 1. For layer operations that affect multiple
hives, each affected hive's generation number is incremented
independently.
...
across any layer, the key is orphaned (see the Deletion section).
...
the fd returns structured event records (see the Watch Model
section for event format).
...
the standard backup format (see the Backup Format section). The
...
**Behaviour:** See the Backup Format section. Privilege check only
