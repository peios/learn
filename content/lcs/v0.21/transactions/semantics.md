...
**Source-scoped.** A transaction binds to a source on its first
mutating operation (value write, key create, key delete, key hide,
SD change, blanket tombstone). Read operations within a transaction
do NOT bind it. All subsequent operations MUST target keys backed
by the same source. An operation targeting a different source fails with
EXDEV. Since a source may back multiple hives (loregd backs both
`Machine\` and `Users\`), a transaction can span multiple hives
backed by the same source.
...
(default 30 seconds). See the Self-Configuration section.
