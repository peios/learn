...
| Right | Value | Description |
|---|---|---|
| ACCESS_SYSTEM_SECURITY | 0x01000000 | Read or modify the SACL. Gated by SeSecurityPrivilege (see the KACS v0.20 Privileges section). |
...
algorithm (see the SD Inheritance section of the KACS v0.20
specification).
...
the KACS v0.20 Privileges section).
...
open time (semantic rule 5). The admin's recourse for revoking
...
on the key object (semantic rule 4). See the Values section for the
