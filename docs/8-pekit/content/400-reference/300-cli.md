...
`clean`, `gen`, `verify`, or `workspace`. Flags may appear before or after the command word and
...
| Command | version selection | local source | `--no-build` | `--no-verify` | `--env` | `--keyring` | `--refresh-source` | `--allow-unanchored` | `--all` | clean mode |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
