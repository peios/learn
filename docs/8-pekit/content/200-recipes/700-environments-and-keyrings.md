...
type: concept
description: "How pekit assembles the environment a target runs in — recipe [env], env.pekit.toml files selected with --env, [wrap] wrappers, and dependency_provider — and how it supplies secrets and keys through keyring files and --keyring flags, exported as PEKIT_KEYRING_* variables."
related:
