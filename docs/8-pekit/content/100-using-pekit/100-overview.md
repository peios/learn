...
Everything in the Peios tree is built with pekit. The kernel, the registry store, the init system, the package manager itself — each is a recipe, and `pekit package` produces the `.peipkg` files that [peipkg-compose](~peios/package-management/composing-a-root) and [peiso](~peiso/building-images/overview) assemble into a running system.
...
`pekit.toml` is the recipe proper — it declares the [source](~pekit/recipes/sources), the [build/test/install/clean targets](~pekit/using-pekit/commands-and-targets), and shared [environment](~pekit/recipes/environments-and-keyrings). The [package files](~pekit/recipes/packages) describe how staged build output is mapped into one or more `.peipkg` payloads. A [`workspace.pekit.toml`](~pekit/workspaces/workspaces) one level up turns a directory of recipes into a **workspace** that pekit can drive as a unit.
...
The [Commands and targets](~pekit/using-pekit/commands-and-targets) page covers what each command selects, its version behaviour, and how target selection works.
...
- [**peiso**](~peiso/building-images/overview) turns a composed root into a bootable image.
...
To use pekit against an existing recipe, read [Commands and targets](~pekit/using-pekit/commands-and-targets) and [Invocation and flags](~pekit/using-pekit/invocation).
