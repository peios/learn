...
type: reference
description: The per-thread operations a program uses directly — reading a thread's ID, the exit-notification mechanism behind thread joins, and thread-local storage. The thread-level companion to the process and thread model.
related:
