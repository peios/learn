...
This path exists because of a bootstrapping problem: the platform services that *make* the normal token flow possible cannot use it, because they have to start *before* it exists. `registryd`, `authd`, `lpsd`, and `eventd` all run as SYSTEM and are all minted this way during [boot](~peios/peinit/boot-and-boot-modes), before authd is available to mint anything.

> [!NOTE]
> There is no allow-list restricting which services *may* be `SYSTEM`. The security boundary is the [registry key descriptor](~peios/peinit/who-can-manage-a-service) on `Machine\System\Services\` — an administrator who can create service definitions is already trusted to assign any identity, so a second gate would add nothing. The dangerous case is never *implicit*, though: `SYSTEM` must be written out explicitly; an empty `Identity` defaults to the minimal `LocalService`, never to SYSTEM.
...
| **[Ad-hoc jobs](~peios/peinit/jobs-and-operations)** | The token captured by JFS — the submitter's (possibly impersonated) identity. |
...
`Identity` decides what a service *can do*; the [ServiceSecurity descriptor](~peios/peinit/who-can-manage-a-service) decides *who can manage it* — two independent concerns. Read [Who can manage a service](~peios/peinit/who-can-manage-a-service) for the other half.

To see how the token is installed alongside the rest of the process setup, read [The execution environment](~peios/peinit/execution-environment).
