...
A **Simple** service is a long-running daemon. peinit forks, installs the [token](~peios/peinit/identity-and-privileges), and execs the binary. The process *is* the service: while it runs, the service is running; when it exits, the service has stopped. This is the default and covers the large majority of services — `registryd`, `authd`, `sshd`, application daemons.

The interesting question for a Simple service is *when does it count as started* — the moment its [dependents](~peios/peinit/dependencies) are allowed to begin. That is the **readiness** model, set by the `Readiness` field.
...
Once ready, a Simple service transitions to **Active** and stays there until its process exits or it is stopped. What happens on exit — clean exit, crash, restart — is the subject of [The service lifecycle](~peios/peinit/the-service-lifecycle) and [Keeping services running](~peios/peinit/supervision).
...
- **Any other exit, or death by signal** → the service **failed**, and the [failure policy](~peios/peinit/supervision) applies.
...
If a legacy binary insists on daemonising, the fix lives at the [packaging](~peios-packages/getting-started/overview) layer — wrap it so it stays in the foreground (commonly a `--no-daemon`/`--foreground` flag) — not at the init layer.

## Where to start

To control *when* either type of service starts, read [Triggers and timers](~peios/peinit/triggers-and-timers).

To follow a service from definition to running process and read its states, read [The service lifecycle](~peios/peinit/the-service-lifecycle).

To configure restart, health checks, and watchdogs for a Simple service, read [Keeping services running](~peios/peinit/supervision).
