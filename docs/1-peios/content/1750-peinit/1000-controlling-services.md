...
Underneath, `peiosctl` is a thin client over peinit's **control socket** at `/run/peinit/control.sock`. The wire protocol — not the CLI — is the normative interface, so everything here (commands, rights, semantics) holds regardless of which front-end you use.
...
- **Every command is access-controlled.** When you connect, peinit captures your [token](~peios/peinit/identity-and-privileges) from the kernel and runs [AccessCheck](~peios/access-decisions/overview) against the target service's descriptor for *every* command. There is no "trust localhost," no override. Who may do what is the subject of [Who can manage a service](~peios/peinit/who-can-manage-a-service).
- **Lifecycle commands create [operations](~peios/peinit/jobs-and-operations).** A `start`/`stop`/`restart`/`reload`/`reset` returns an `operation_id` — a GUID you can poll. Conflict resolution between concurrent commands happens at the operation layer, which is why two simultaneous `start`s merge instead of colliding.
...
| Command | Does | Required right |
|---|---|---|
| `shutdown <type>` | Graceful [shutdown](~peios/peinit/shutdown). `type` is `poweroff`, `reboot`, or `halt`. | `SYSTEM_SHUTDOWN` |
...
A command sent to a service in an unexpected state returns an **error**, not a silent no-op. This matrix is the authority on what each command does in each [state](~peios/peinit/the-service-lifecycle):
...
- `state` and `cause` are the [lifecycle](~peios/peinit/the-service-lifecycle) pair — read them together.
- `status_text` is the latest `STATUS=` string the service sent via sd_notify (`null` if never sent; cleared on each restart).
- `current_job` and `current_operation` are the [job and operation](~peios/peinit/jobs-and-operations) GUIDs, or `null`.
- `health` is `healthy`, `unhealthy`, `unknown`, or `null` (no health check).
- `definition_removed` is `true` when the definition was deleted but an instance is still [draining](~peios/peinit/defining-a-service).
...
| `UNKNOWN_SERVICE` | No such service definition (also returned for `start`/`restart`/`reload` on a [definition-removed](~peios/peinit/defining-a-service) service). |
...
To understand the states the matrix refers to, read [The service lifecycle](~peios/peinit/the-service-lifecycle).

To understand the operations every lifecycle command creates and how to poll them, read [Jobs and operations](~peios/peinit/jobs-and-operations).

To configure who may run each command, read [Who can manage a service](~peios/peinit/who-can-manage-a-service).
