...
A service is a *securable object*. Just as a [file](~peios/file-access/overview) or a [registry key](~peios/the-registry/access-control) carries a [security descriptor](~peios/security-descriptors/overview) that decides who may touch it, a service carries one that decides who may **start, stop, query, or reload** it. peinit enforces it on *every* [control command](~peios/peinit/controlling-services) — there is no command that skips the check.

This is the second, independent half of the security model. The first half, [Service identity and privileges](~peios/peinit/identity-and-privileges), is about what a service *can do* (its token). This page is about who can *manage the service* — a completely separate question, answered by a completely separate descriptor.
...
|---|---|---|---|
| **Registry key SD** | Who can *read or edit the definition*? | On the `Machine\System\Services\<name>` key | [LCS](~peios/the-registry/access-control), at key-open time |
...
The registry key SD is ordinary [registry access control](~peios/the-registry/access-control) and not peinit's concern — peinit reads definitions as SYSTEM, which has full access. The **ServiceSecurity** SD is peinit's domain, and the rest of this page is about it.
...
1. peinit captures the caller's [token](~peios/peinit/identity-and-privileges) from the kernel (`kacs_open_peer_token`) — the caller's *effective* identity at connection time, so if the caller is [impersonating](~peios/impersonation/overview), the impersonated identity is what is checked.
...
ServiceSecurity is **hot-reloaded**: a change to the value in the registry takes effect on the **next control request**, with no service restart. peinit picks the change up through a [registry notification](~peios/the-registry/watches). This is why ServiceSecurity is in its own [mutability class](~peios/peinit/defining-a-service) — access policy should be able to change without disturbing a running service.
...
For the *other* half of the security model — what a service can reach once it is running — read [Service identity and privileges](~peios/peinit/identity-and-privileges).

For the commands these rights gate, read [Controlling services](~peios/peinit/controlling-services).
