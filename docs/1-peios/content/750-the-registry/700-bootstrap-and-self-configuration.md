...
Every operational parameter the registry uses has a **compiled-in default** that produces correct behaviour. From the moment the kernel module loads, the registry runs on those defaults — it is immediately operational and never blocks waiting for configuration. The [base layer](~peios/the-registry/what-layers-are-for) likewise exists unconditionally, hardcoded, needing no persisted state. So before any store has registered, the machinery is already alive; it simply has no hives to serve yet.
...
At no point is there a stall. Empty store, missing keys, seed arriving later — each is handled by "use the defaults until something better shows up", driven by the same [watch](~peios/the-registry/watches) mechanism every other reactive consumer uses.
...
If you want the validation discipline this page leans on, read [Configuration, not storage](~peios/the-registry/configuration-and-meaning).

If you want the change-notification mechanism behind the self-watch, read [Watching for changes](~peios/the-registry/watches).
