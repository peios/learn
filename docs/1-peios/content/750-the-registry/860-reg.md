...
title: reg
type: reference
description: "reg is the command-line tool for the live registry: it reads and writes values, keys, layers, links, security descriptors, watches, and atomic batches, going through the kernel on every operation. It is the scripting counterpart to regman — reg changes what a setting is, regman documents what it means."
related:
...
`reg` is the command-line interface to the **live** registry — the running [LCS](~peios/the-registry/lcs-and-sources) store, reached through the registry system calls. Where [`regman`](~peios/the-registry/regman) reads shipped documentation and tells you what a key *means*, `reg` talks to the kernel and reads or writes what a key *is*. It is the everyday tool for scripting configuration: querying effective values, writing to layers, masking and hiding, managing security descriptors, watching for change, and taking backups.

```
reg <command> [options] <key> [value] [data]
```

`reg` is the read/write half of the registry toolset and [`regman`](~peios/the-registry/regman) is the lookup half. The division is exact:

| To… | Use… |
|---|---|
| Read or change what a value is *set to*, on this machine, right now | `reg` |
| Look up what a value *means* — its type, default, valid range, when it applies | [`regman`](~peios/the-registry/regman) |
...
Only the *winning* value is shown; the full shadowed stack beneath it is not yet exposed by any client ABI (the `-L` flag will render the whole stack unchanged once that primitive lands). See [Layers](~peios/the-registry/layers) for what "effective" resolves against.
...
See [Deleting keys and values](~peios/the-registry/deleting-keys-and-values) for how a per-layer delete differs from a tombstone.

---

## Masking and hiding

These commands expose LCS's tombstone and hide primitives, which *mask* lower-precedence state rather than removing a layer's own entry — the distinction that makes [layers](~peios/the-registry/layers) revertible. The layer is chosen with `--layer` (default: base).
...
Managing per-process **private** layer views is out of scope for `reg`; see [private hives and layers](~peios/the-registry/private-hives-and-layers).
...
A security-descriptor change takes effect on future opens only (existing handles keep the access mask they were granted). **Security changes are not layered and are not undone by layer removal** — they mutate the key object directly. See [Access control on keys](~peios/the-registry/access-control).
...
`get` and `info` follow symlinks to their target by default; pass `--no-follow` to inspect the link key itself. See [Advanced: registry links](~peios/the-registry/registry-links).
...
See [Watching for changes](~peios/the-registry/watches) for why a service watches instead of polling.
...
Apply a batch of operations to one hive in a single [transaction](~peios/the-registry/transactions) — all-or-nothing. All operations must target one hive (a cross-hive batch fails with exit `4`). `-` reads standard input.
...
See [Backup and restore](~peios/the-registry/backup-and-restore) for when to reach for each path.
...
- [`regman`](~peios/the-registry/regman) — the registry *manual*: what a key or value *means*, as opposed to what it is set to. Consult it before you `reg set`.
- [Keys, values, and types](~peios/the-registry/keys-values-and-types) — the data model `reg` addresses.
- [Layers](~peios/the-registry/layers) — what "effective" resolves against, and what `--layer`, `mask`, and `hide` operate on.
- [Access control on keys](~peios/the-registry/access-control) — the check every `reg` operation goes through.
