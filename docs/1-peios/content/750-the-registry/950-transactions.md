...
This is why [role installation](~peios/the-registry/what-layers-are-for) is clean. A role's entire configuration — its keys, its values, its layer — is written inside one transaction and committed as a unit. The role is never observed half-installed: either the whole role is there or none of it is. (Removing a role is the *other* mechanism — deleting its layer — covered with [layers](~peios/the-registry/what-layers-are-for).)
...
- **One store at a time.** A transaction is scoped to a single hive's store; it cannot span hives backed by different [sources](~peios/the-registry/lcs-and-sources). Atomicity across separate stores is not offered.
...
For the recency rule that settles competing writes, read [Layers](~peios/the-registry/layers).
