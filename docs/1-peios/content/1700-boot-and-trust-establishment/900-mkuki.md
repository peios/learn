...
title: mkuki
type: reference
description: Build a UEFI unified kernel image by appending the .cmdline, .linux, and .initrd sections onto a PE/COFF EFI stub, producing the single EFI binary that firmware boots directly — with an optional resident watch mode.
related:
