...
# swap the directories (brief gap between the two renames)
sudo mv /var/www/pkgs.example.org{,.old}
sudo mv /var/www/pkgs.example.org{.new,}
sudo rm -rf /var/www/pkgs.example.org.old
```

Note the two renames leave a brief window where the path does not exist; a truly atomic swap needs the served root to be a symlink you repoint with `ln -sfn`.
...
- Signing key: see [Signing keys](~peios-packages/running-a-farm/signing-keys#backups). Offline, separate medium.
