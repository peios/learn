...
**Tombstones** are the tool for "hide, don't delete": a per-value tombstone masks lower layers for one value, and a blanket tombstone masks all lower values of a key on a layer at once. Keys have the same idea via [hide](~peios-sdk/reference/registry#deleting-and-hiding-keys).
...
This SDK is the registry **client** — it reads and writes the store. It does not implement a registry **source** (a storage backend); that's a separate library, [**librsi**](~peios-sdk/registry-sources/overview). As a client you speak only the calls in [`registry.h`](~peios-sdk/reference/registry).
...
- **[`registry.h` reference](~peios-sdk/reference/registry)** — every call in full.
