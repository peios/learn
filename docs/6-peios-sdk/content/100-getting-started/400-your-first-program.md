...
This is a complete program you can compile and run. It does not touch the kernel — it works entirely with the security-descriptor *vocabulary* from `<peios/security.h>`, which makes it a safe first outing: no privileges, no live tokens, just the conventions from [Library conventions](~peios-sdk/getting-started/library-conventions) put to work.
