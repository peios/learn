...
|---|---|---|
| **SID** | The unique binary name of a principal — a user, group, machine, or well-known system actor. | [`security.h`](~peios-sdk/reference/security#sids) |
| **Security descriptor (SD)** | What protects an object: its owner, its group, and the ACLs that grant or deny access. Built from ACEs, each naming a SID and an access mask. | [`security.h`](~peios-sdk/reference/security#building-security-descriptors) |
| **Token** | The runtime object that carries an identity — a user SID, groups, privileges, an integrity level, claims. Every access decision is made *against a token*. A token is a file descriptor. | [`token.h`](~peios-sdk/reference/token) |
| **Access check** | The act of deciding whether a token may perform a desired access on an object, given the object's SD. | [`access.h`](~peios-sdk/reference/access) |
...
1. **Get the subject's token.** Usually the caller's — often via [`peios_token_open_peer`](~peios-sdk/reference/token#opening-and-creating-tokens) on a socket, so you learn who connected — or your own effective token (`token_fd = -1`).
2. **Get the object's security descriptor.** You either hold it already, read it off a file with [`peios_file_get_sd`](~peios-sdk/reference/file), or build one with a [`peios_sd_builder`](~peios-sdk/reference/security#building-security-descriptors).
3. **Run the check.** [`peios_access_check`](~peios-sdk/reference/access#the-check) tells you whether the desired rights are granted, and exactly which subset was granted.
...
For the exhaustive per-function detail behind any of these, the [reference section](~peios-sdk/reference/security) documents every symbol.
