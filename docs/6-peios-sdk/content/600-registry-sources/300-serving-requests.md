...
With a [source fd in hand](~peios-sdk/registry-sources/registering-a-source), a source spends its life in one loop: read a request, decode it, do the work, reply. This guide builds that loop. The exhaustive per-op detail is in [`rsi/request.h`](~peios-sdk/reference/rsi-request) and [`rsi/response.h`](~peios-sdk/reference/rsi-response).
...
- **[Building responses](~peios-sdk/registry-sources/building-responses)** — choosing and filling the right reply.
- **[`rsi/request.h` reference](~peios-sdk/reference/rsi-request)** — every op's decoder and struct.
